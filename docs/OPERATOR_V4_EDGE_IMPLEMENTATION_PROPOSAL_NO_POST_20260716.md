...
- exact OCO取消transportの直前と、その後のposition-specific MARKET time exit
  transportの直前に、executor-owned monotonic clockで最大2秒以内の公式public status
  `OPEN` evidenceを、それぞれ別のone-use evidenceとして消費する。
  OCO取消前に`CLOSE`、`MAINTENANCE`、unknown、staleとなった場合はOCOを取消さず
  persistent HALTとする。OCO取消後の決済前に同条件となった場合はclose transportを
  行わずpersistent HALTとし、fresh reconciliation後の単発risk-reducing action以外へ進まない。
