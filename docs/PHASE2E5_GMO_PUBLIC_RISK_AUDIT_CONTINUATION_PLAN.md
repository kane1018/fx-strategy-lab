...
## 16. Phase 3A準備ロードマップとの関係

Phase 3A準備として、Private API read-only、APIキー / secret管理、Live Verification Mode、
...
ただし、Phase 3A準備はPhase 2E-5を飛ばすためのものではない。Phase 2E-5では引き続きPublic APIのみ、
`gmo-public / USD_JPY / M1 / steps 5 / --enable-shadow-risk`、manual only、1日1回までの条件で短期確認を続ける。
