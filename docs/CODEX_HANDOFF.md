...
- 現在のフェーズ: **Phase 2G Public shadow risk/auditオフライン最終デバッグ監査完了 / Phase 3B read-only公式仕様確認・実装設計へ進行可**。Phase 2E-4Rでは直近kline条件の
  ...
  **A: Phase 3B read-only公式仕様確認・実装設計へ進んでよい**。詳細は
  ...
  Phase 3B read-only実装は、公式仕様確認・実装設計後の別タスクに限定する。
