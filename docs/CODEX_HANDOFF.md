...
- 現在のフェーズ: **Phase 3B-0 Private API read-only公式仕様確認・実装設計完了 / Phase 3B-1 mocked read-only skeleton候補**。Phase 2E-4Rでは直近kline条件の
  ...
  **A: Phase 3B read-only公式仕様確認・実装設計へ進んでよい**。詳細は
  ...
  次に進む場合はPhase 3B-1としてmocked read-only skeleton / auth helper / schemas / no-order-import testsから開始する。
