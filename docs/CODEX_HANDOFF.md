...
- 現在のフェーズ: **Phase 3B-1 mocked private readonly skeleton実装完了 / Phase 3B-2 mocked endpoint tests候補**。Phase 2E-4Rでは直近kline条件の
  ...
  **A: Phase 3B read-only公式仕様確認・実装設計へ進んでよい**。詳細は
  ...
  注文API、実注文、実資金には進んでいない。次に進む場合はPhase 3B-2としてread-only endpointごとの
  mocked tests / sanitizer拡張を別タスクで扱う。
