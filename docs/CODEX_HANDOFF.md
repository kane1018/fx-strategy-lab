...
- 現在のフェーズ:
  **E1_SHADOW_FULL_AUTO_ENGINE_NO_POST 実装済み・ゲート未通過／local-only・offline・no-POST**。
  2026-07-10、二軌道モデルのインフラ軌道 E1 として、有限の offline run 内に限り
  decision → fixed risk gate → virtual entry / settlement → intent journal → reconcile → audit →
  kill / dead-man → restart protocol を `backend/app/shadow/e1/` に隔離実装した。
  仮説軌道は E1 / E2 に留め、E1 の結果を edge、performance proof、live readiness、E2/E3 permission
  に変換しない。実装済みは **ゲート合格を意味せず**、必要稼働期間・イベント数・fault injection・
  kill/dead-man 証拠は今後の bounded offline run で蓄積する。
  API / broker / network / credential / env / `.env` / actual POST は使用せず、生成する journal / state /
  audit / gate evidence は ignore 済み `backend/shadow_exports/e1/` 配下だけに置き commit しない。
  過去の Step 6G / live 記録は、上記インシデント注記のとおり simulation / docs claim / unknown を含み、
  **E1 の実装証拠・ゲート証拠には算入しない**。
  設計・状態は [E1設計契約](E1_SHADOW_FULL_AUTO_ENGINE_NO_POST.md)、有限run手順は
  [E1 runbook](E1_SHADOW_FULL_AUTO_ENGINE_RUNBOOK_NO_POST.md)、将来のE2→E3 review用仕様表は
  [sanitized API capability sheet](API_CAPABILITY_SHEET_SANITIZED_NO_POST.md) を参照する。
  status は `E1_IMPLEMENTED_NOT_GATE_PASSED` / `performance_proof_status=false` /
  `live_ready=false` / `unattended_live_supported=false`。
  実装時検証: focused E1 **111 passed**、E1 + production isolation **25 passed**、
  existing shadow regression **125 passed**、backend Ruff clean。これは実装回帰の結果であり、
  2週間 / 10営業日等のE1→E2運用ゲート証拠ではない。
- 直前フェーズ:
  **OPERATOR_BRIEFING_READONLY_CLI_NO_POST
  read-only の最小 CLI を自走実装（"UI"＝端末で参照するツール）／live/broker/credential/POST は対象外**。
  2026-07-09、operator 承認のもと "UI は未完成→最小 read-only CLI が推奨" を受けて CLI を自走実装:
  `backend/app/cli/operator_briefing_cli.py`（+ `app/cli/__init__.py`・tests 8件）＝
  `python -m app.cli.operator_briefing_cli [flags]`。argparse で **SAFE ラベルを引数で受け**（内部状態から手入力・
  取得しない）→ 既存純粋関数（supply→briefing→session）→ **warning-first テキストを stdout に表示**。
  `--decision {BUY,SELL,HOLD,NO_ACTION}` は **operator 自身の記録用**（`--reason` 必須・system は決定しない）・
  未指定/不明は fail-closed で caution 化。**ローカル I/O のみ**（args in / text out）・no network/venue/env/secret/
  fetch/POST・SERE 非起動・direction/confidence/推奨なし・no-flag≠permission。module isolation で
  network/venue/env/execution/broker/credential/POST/fetch 不在を確認。運用手順 doc に CLI 実行例を追記。
  **注記: "UI" と呼べるのは端末 CLI まで**（GUI/Web dashboard は defer・PULL/warning-first を崩さないため）。
  full suite **7302 passed** / ruff clean。実POST/broker write/private GET/fetch/credential/env read=ゼロ・raw値/ID/CSV commit なし。
  status 全不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false）。範囲外（自走不可・operator 主導/DEFER/前提未達）: 実口座反映(private GET)・
  live/actual POST・SERE 実行・prescriptive preview・confidence・auto-trade・収益 edge。
- 直前フェーズ:
  **OPERATOR_MANUAL_TRADING_SUPPORT_READONLY_COMPLETE_NO_POST
  read-only の"手動運用支援"を自走で仕上げ（session helper + 運用手順 doc）／live/broker/credential/POST は対象外**。
  2026-07-09、operator 承認（A）のもと自走実装:
  `backend/app/services/operator_briefing_session.py`（+ tests 14件）＝
  `start_operator_briefing_session(request)->OperatorBriefingSession`（decision=PENDING・briefing safe summary 付）／
  `record_operator_decision(session, OperatorDecisionLabel, reason)`（**operator が渡した決定を"記録"のみ**・
  system は決定を生成/導出しない・PENDING/空 reason は fail-closed で ValueError）／
  `render_operator_briefing_session`（warning-first briefing + [SESSION LOG] ブロック・禁止フラグメント guard）。
  `OperatorDecisionLabel`=PENDING / OPERATOR_DECIDED_ENTRY_BUY|SELL|HOLD|NO_ACTION（**operator 自身の記録用ラベル**）。
  BriefingSafeSummary は label/count/bool のみ・never truthy・perf_proof/live=False・direction/confidence/raw/ID なし。
  運用手順 doc [OPERATOR_MANUAL_TRADING_SUPPORT_PROCEDURE_NO_POST.md](OPERATOR_MANUAL_TRADING_SUPPORT_PROCEDURE_NO_POST.md) 追加
  （PULL/warning-first・SAFE ラベルは内部から手入力・実弾は operator の外部環境 or 別 operator-gated one-POST・
  no-flag≠permission・"規律であって収益装置でない"）。
  **A（read-only 手動運用支援）完成**。module isolation（network/venue/env/execution/broker/credential/POST/fetch 不在）。
  full suite **7294 passed** / ruff clean。実POST/broker write/private GET/fetch/credential/env read=ゼロ・raw値/ID/CSV commit なし。
  **範囲外（operator 主導/DEFER/前提未達）**: 実口座反映(runtime private GET)・live/actual POST・SERE 実行・
  prescriptive preview・confidence・auto-trade・収益 edge。system は自律で broker/credential/POST に触れない。
  status 全不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false）。
- 直前フェーズ:
  **OPERATOR_PRE_TRADE_BRIEFING_READONLY_COMPLETE_NO_POST
  read-only decision-support briefing を"完成"まで自走（façade + runbook）／live/auto は対象外**。
  2026-07-09、operator 承認（"read-only 完成まで自走"）のもと残りを自走実装:
  read-only orchestration façade `backend/app/services/operator_pre_trade_briefing_service.py`（+ tests 9件）＝
  `produce_operator_pre_trade_briefing(request)->OperatorPreTradeBriefingBundle` /
  `render_operator_pre_trade_briefing(request)->str` で **supply(fail-closed 正規化)→generate(warning-first/
  NO_ACTION default)→render** を1呼び出しに束ねる純粋関数。combined render は **[0]入力データ品質cautions→
  [1..9] warning-first briefing** ＋ **禁止フラグメント guard**。bundle は never truthy・perf_proof/live=False・
  direction/confidence/raw/ID フィールドなし。read-only 使い方 runbook
  [OPERATOR_PRE_TRADE_BRIEFING_READONLY_USAGE_NO_POST.md](OPERATOR_PRE_TRADE_BRIEFING_READONLY_USAGE_NO_POST.md) を追加。
  **完成した"read-only decision-support"の範囲**: SafeLabelSupplyRequest→briefing→render の read-only pipeline。
  **範囲外(operator-gate/DEFER/前提未達)**: 実口座反映(runtime private GET)・live/actual POST・SERE 実行・
  prescriptive preview・confidence・auto-trade・収益 edge。SAFE ラベルは **INTERNAL safe state から caller が供給**
  （broker 照会はこのパイプラインでは行わない・真の照合は別 operator-gated read-only Step）。
  module isolation（network/venue/env/execution/broker/credential/POST/fetch 不在）。full suite **7280 passed** / ruff clean。
  実POST/broker write/private GET/fetch/credential/env read=ゼロ・raw値/ID/CSV commit なし。
  status 全不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false）。
- 直前フェーズ:
  **OPERATOR_BRIEFING_SAFE_LABEL_SUPPLY_IMPLEMENTATION_NO_POST_READONLY
  briefing への SAFE ラベル供給層を read-only 実装（純粋関数・fail-closed・no-private-GET・no-POST）**。
  CASE=`SAFE_LABEL_SUPPLY_FOR_OPERATOR_CAUTION_BRIEFING_IMPLEMENTED_NO_POST_READONLY`。
  2026-07-09、caution briefing generator に渡す入口を実装:
  `backend/app/services/operator_briefing_safe_label_supply.py`（+ tests 19件）。
  `SafeLabelSupplyRequest`（caller の **SAFE ラベル文字列/safe count/文脈ラベルのみ**・raw price/spread/PnL/size・
  ID・direction・ENTRY を持たない）→ `build_briefing_inputs` → `SafeLabelSupplyResult(briefing_inputs, supply_cautions,
  input_completeness)`。**fail-closed**: unknown/missing/invalid → `*_UNKNOWN` + supply caution／uncertainty 不明→high／
  pending 不明→caution（present で count 不明→"at least one"=1）。**文脈照合**: 既知 rejected ラベル→タグ→briefing で
  RESEMBLES_REJECTED、`OUTSIDE_TESTED_SCOPE`→非ledger marker→OUTSIDE(caution)、`UNKNOWN_CONTEXT`→NOT_ASSESSED、
  未知ラベル→OUTSIDE+caution。**いずれも "no-flag≠permission"・signal でも方向でもない**。
  convenience `build_caution_briefing_from_labels`→(result, briefing)。
  併せて briefing を**最小 fail-closed 強化**（event 不明 / budget 不明 → hard-stop 追加。既存挙動は不変・membership test 維持）。
  純粋・決定論・**never truthy**・perf_proof/live=False・出力に raw/ID/direction/confidence フィールドなし・
  **ENTRY 非生成・SERE 非起動・briefing 執行なし**。module isolation（network/venue/env/execution/broker/credential/
  POST/fetch 不在）。full suite **7271 passed** / ruff clean。実POST/broker write/private GET/fetch/credential/env read=ゼロ・
  raw値/ID/CSV commit なし。status 全不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false /
  live_ready=false / unattended_live_supported=false）。AUTO_PREVIEW_SIGNAL/confidence/live budget/auto-trade は DEFER 継続。
- 直前フェーズ:
  **OPERATOR_PRE_TRADE_CAUTION_BRIEFING_IMPLEMENTATION_NO_POST_READONLY
  凍結設計の read-only 実装（純粋関数・safe-aggregate・no-private-GET・no-POST）**。
  2026-07-09、凍結設計(commit 57ce9cc)を基準に briefing generator を実装:
  `backend/app/services/operator_pre_trade_caution_briefing.py`（+ tests
  `app/tests/test_operator_pre_trade_caution_briefing_no_post.py`・16 tests）。
  `generate_caution_briefing(BriefingInputs)->CautionBriefing`：**caller が渡す SAFE ラベルのみ**を入力に
  （internal safe state・**取得/broker/price 読取なし**）、**warning-first・NO_ACTION default・no-flag≠permission・
  方向/確信度/推奨なし**の briefing を生成。**hard-stop 検出**（spread異常/event近接/uncertainty高/budget超過/
  internal state unknown/execution not ready）で NO_ACTION を強調。**tested-scope/rejected-ledger(H-01..H-05 タグ)
  照合は caution のみ**（RESEMBLES_REJECTED / OUTSIDE_TESTED_SCOPE / NOT_ASSESSED・いずれも "no-flag≠permission"・
  signal でも方向でもない）。`render_caution_briefing` は **warning-first 順序**＋**禁止フラグメント guard**（推奨が漏れたら ValueError）。
  純粋・決定論・**never truthy**・`performance_proof_status=False`/`live_ready=False` ハードコード・
  出力に raw price/spread/PnL/size・ID フィールドなし。module isolation test（network/venue/env/execution/broker/
  fetch/POST 不在）。**ENTRY_BUY/ENTRY_SELL/HOLD は生成しない（operator safe label のまま）・SERE 非起動・briefing は執行しない。**
  AUTO_PREVIEW_SIGNAL/confidence/live budget/auto-trade は **DEFER 継続**。full suite **7250 passed** / ruff clean。
  実POST/broker write/private GET/fetch/credential/env read=ゼロ・raw値/ID/CSV commit なし。
  status 全不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false）。設計基準: [design](OPERATOR_PRE_TRADE_CAUTION_BRIEFING_DESIGN_NO_POST_20260709.md)。
- 直前フェーズ:
  **OPERATOR_PRE_TRADE_CAUTION_BRIEFING_DESIGN_NO_POST
  warning-first / PULL式 pre-trade caution briefing の設計凍結 / docs-only / no-POST**。
  CASE=`OPERATOR_DECISION_SUPPORT_RESCOPED_TO_WARNING_FIRST_CAUTION_BRIEFING_NO_POST`。
  2026-07-09、「売買判断は operator・AI/システムは判断材料のみ」方針を Fable5案（decision-support でなく
  **pre-trade caution**）で採用し**設計を凍結**（docs のみ・実装/UI/dashboard/broker/fetch/backtest/live/POST なし）。
  中身: warning-first・**PULL式（system は好機を PUSH しない）**・**NO_ACTION default**・**no-flag≠permission**・
  no validated edge / not advice を毎回表示・最終判断は operator・**ENTRY_BUY/ENTRY_SELL/HOLD は operator safe label
  のまま（AI 代行なし）**・confidence/alpha/expected profit/win-rate/direction/recommendation を出さない。
  出してよい: 恒久disclaimer/NO_ACTION status/hard-stop該当/**tested-scope & rejected-ledger(H-01..H-10)照合**/
  risk・exposure・budget の**内部 safe 状態(no runtime private GET)**/market-state 記述(方向なし)/checklist/
  uncertainty/operator 最終判断+理由記録。レポート順序=warning-first 固定。
  **rejected 照合は caution のみ生成し signal 化しない**("非棄却=許可"とも読ませない・未検証領域は unknown/no edge)。
  **briefing は SERE を起動しない・実 POST は別 Step(current-turn exact confirmation・max one POST/no retry/no repost)**。
  AUTO_PREVIEW_SIGNAL_BUY/SELL・confidence・live budget・auto-trade は **DEFER**(validated edge+operator承認まで)。
  status 全不変(CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false)。code/test/config/取得/POST なし・raw値/ID/credential 非露出・CSV commit なし。
  記録: [OPERATOR_PRE_TRADE_CAUTION_BRIEFING_DESIGN_NO_POST_20260709.md](OPERATOR_PRE_TRADE_CAUTION_BRIEFING_DESIGN_NO_POST_20260709.md)。
- 直前フェーズ:
  **RESEARCH_PLATFORM_PRODUCTIZATION_NO_POST
  reusable research platform organized / prescriptive preview・confidence・live DEFERRED / docs-only / no-POST**。
  2026-07-09、operator が Fable5案（productization 主軸・adaptive の安全部分のみ吸収・危険部分は defer）を採用。
  目的は **adaptive alpha engine / confidence 付き signal preview を作ることではなく**、構築済みの
  評価・安全・統治・記録基盤を再利用可能 platform として索引化すること。**明示 DEFER（validated edge＋operator
  承認まで作らない）**: prescriptive `AUTO_PREVIEW_SIGNAL_BUY/SELL` / confidence・uncertainty / live research budget /
  auto-trade / regime を live 判断入力にすること（regime は研究ラベル用途のみ）。理由=validated signal 無しの
  confidence は"ノイズへの確信"＝誤誘導/裁量化/HARKing の温床。
  整理物: platform 索引（評価gate/false-positive rejection/backtest基盤/data取込/公開GET client/
  **safe execution template=default-deny hard guard・max one POST・no retry/repost・settlement route分離**/
  governance/reporting/**記述的 market-state observer（BUY/SELL出さず・NO_ACTION層まで）**）＋
  **hypothesis registry（H-01..H-10・多重検定台帳・null #1/3）**。自律境界: AI=状態記述/NO_ACTION/registry/backtest gate/記録、
  operator確認必須=全live POST・全更新・データ取得・DEFER層有効化。ENTRY_BUY/SELL/HOLDはoperator safe labelのまま。
  status不変（CLOSED_OUT / NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE / perf_proof=false / live_ready=false /
  unattended_live_supported=false）。docs のみ・取得/採点/live なし・raw値/CSV commitなし。
  記録: [RESEARCH_PLATFORM_PRODUCTIZATION_NO_POST_20260709.md](RESEARCH_PLATFORM_PRODUCTIZATION_NO_POST_20260709.md) /
  [HYPOTHESIS_REGISTRY_NO_POST.md](HYPOTHESIS_REGISTRY_NO_POST.md)。
- 直前フェーズ:
  **CROSS_ASSET_RATES_LEADLAG_PREREGISTRATION_NO_POST
  mechanism-first pre-registration (rates->USDJPY) / BLOCKED pending operator rate-data / docs-only / no-POST**。
  2026-07-09、operator 指示で closeout 後に **mechanism-first 原則**（data-dredging 禁止・"取れるデータで
  何が試せるか"の列挙を明示的に排除。機構を先に固定→要求データを取りに行く逆順禁止。要求データが
  no-credential で入手不能なら弱機構×易データに流れず closeout 維持）を runbook §6b に明文化。
  その上で最強の a-priori 機構 **CROSS_ASSET_RATES_LEADLAG** を**データを見る前に事前登録（凍結）**:
  機構=USD/JPY は日米金利差/リスクの従属変数で、米金利(2y/10y or rate-futures proxy)の日中変化が
  USD/JPY に**先行**。固定ルール=直前 k=3bar の米金利変化の符号方向に USD/JPY を h=3bar 保有（単一variant・
  スキャン禁止）。USD/JPY 単一取引（1 spread・triangular の多legコストなし）。対照=コスト込みPF>1・
  **lead-lag placebo（金利系列の時間反転/シャッフルで予測性消失）**・contemporaneous(lag0)対照・
  sign-permutation・multi-resolution/3ブロック。要求データ=**US金利の日中M5整合系列を operator が local CSV
  提供（no-credential厳守・repo外・非commit）**、または認証不要公開ソースの明示承認。**私は credentialed 源を取得しない**。
  現 status=**BLOCKED_PENDING_OPERATOR_PROVIDED_RATE_DATA**（提供/承認あれば実装→1回採点=trial null #2/3相当、
  不可なら closeout 維持）。docs のみ・実装/取得/backtest再実行なし・raw値/CSV commitなし。
  performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_CROSS_ASSET_RATES_LEADLAG_PREREGISTRATION_NO_POST_20260709.md](STRATEGY_CROSS_ASSET_RATES_LEADLAG_PREREGISTRATION_NO_POST_20260709.md) /
  [runbook §6b](RESEARCH_RUNBOOK_NO_POST.md)。
  さらに「取得前にまず整理」を実施（**機構×時間軸×データ 実現可能性マップ**・mechanism-first・行=強機構）。
  判明: **日次(EOD)に落とすと no-credential の壁がほぼ消える**（US/JP利回り・USD/JPY・equity/VIX・暦は
  公開無認証で数十年入手可＝検定力も十分）。代償に**日次で試せる強機構(金利差/carry・risk-sentiment・月末)は
  教科書的・裁定済で edge prior 低**。高prior の intraday microstructure lead-lag は逆に分足データが no-credential 壁。
  → 推奨: **intraday金利への即取得は非推奨**（既存M5契約は intraday版として保留）。代わりに
  **日次・公開・無認証データで "金利差/carry→USD/JPY" を mechanism-first で新規事前登録→operator承認public GET→
  日次版標準gateで1回採点**（blocked回避・清潔・十分検定力だが低prior＝null公算）。代替=closeout維持。**operator判断待ち**。
  記録追加: [STRATEGY_MECHANISM_TIMESCALE_DATA_FEASIBILITY_NO_POST_20260709.md](STRATEGY_MECHANISM_TIMESCALE_DATA_FEASIBILITY_NO_POST_20260709.md)。
  operator が(1)を選択 → **日次 rate-differential 単一仮説を凍結事前登録**（US-JP 10y差の変化がUSD/JPYを予測・
  k=5/h=5・2005-2026・cost2.0pip・placebo/contemporaneous/sign-perm/3ブロック・最小標本≥200・single variant・
  post-OOS retuning禁止。commit `44647f8` で凍結・push 済＝**データ接触前に確定**）。
  だが採点用データ取得で **環境ブロッカー**: 当sandbox は GMO host のみ到達可で **FRED/MOF へ到達不可**
  （FRED timeout・MOF 404・proxy無し）。契約§2 に従い **sandbox 無効化・別ソース代替・行スクレイプはせず BLOCKED**。
  data dir は清潔（stray無し）。**解決策 = operator が公開・無認証の3 CSV を自分でDLして repo外 data dir に置く**
  （FRED DGS10 / FRED DEXJPUS / MOF JGB 10y）→ 私は local を parse し凍結契約で1回採点。または closeout 維持。
  status=**BLOCKED_DATA_UNREACHABLE_FROM_SANDBOX（operator が local CSV 提供 or closeout）**。
  記録: [STRATEGY_RATE_DIFFERENTIAL_DAILY_PREREGISTRATION_NO_POST_20260709.md](STRATEGY_RATE_DIFFERENTIAL_DAILY_PREREGISTRATION_NO_POST_20260709.md)。
  **→ operator は B（closeout 維持）を選択。** 日次 rate-differential 契約は **FROZEN_UNEXECUTED**
  （commit `44647f8` に凍結・未採点。将来データ提供時は post-OOS retuning なしで契約通り1回採点可能）。
  **研究フェーズは CLOSED_OUT を維持**。current_strategy_status=NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE /
  performance_proof_status=false / live_ready=false / unattended_live_supported=false（全て不変）。
  多重検定台帳: 本契約は未採点のため null にカウントしない（vol-regime の null #1/3 のまま）。
  他日次仮説・データ探索へは広げない。
- 直前フェーズ:
  **RESEARCH_PLATFORM_CLOSEOUT_AND_STATUS_RECORD_NO_POST
  strategy search CLOSED_OUT / status formally recorded / docs-only / no-POST**。
  2026-07-09、operator 判断で戦略探索を **CLOSED_OUT**（pre-registered null #1/3 で前倒し発動）。
  5仮説を厳密 gate で棄却し **`NO_ROBUST_EDGE_FOUND_IN_TESTED_SCOPE`** を正式確定
  （1. M5 technical / 2. H1 trend-ride execution-friction / 3. SESSION_MOMENTUM sign-perm /
  4. GOTOBI multi-year M5 / 5. VOL_REGIME_CONDITIONAL_BREAKOUT）。status flag 確定:
  performance_proof_status=false / live_ready=false / unattended_live_supported=false /
  unattended_full_auto_completed=false。**docs のみ**（新仮説実装・取得・backtest再実行なし）。
  正式記録: rejected ledger・標準評価gate（`evaluate_under_standard_gate` / `evaluate_gotobi_effect`・
  multi-resolution・sign-permutation・leg別spread・cost2.0×・0.5pip/side・min-sample・多重検定台帳・
  post-OOS retuning禁止）・構築資産（backtest基盤/hardening/取込アダプタ/gotobiモジュール/frozen signal engine）・
  データ在庫（repo外・未commit: H1 15ヶ月, M5 3ヶ月, M5 2.7年198,129 bars）・安全不変則。
  再開用 runbook 整備（新規事前登録→operator承認データ→凍結ルールを標準gateで1回採点、の順のみ）。
  本当の成果物は「偽陽性を再現的に却下できる評価基盤」（gotobi 16件PF5.38 mirage を最小標本gateが却下）。
  実POST/broker write/private GET/credential/env read/取得=ゼロ、raw値/CSV commitなし。
  記録: [RESEARCH_PLATFORM_CLOSEOUT_AND_STATUS_RECORD_NO_POST_20260709.md](RESEARCH_PLATFORM_CLOSEOUT_AND_STATUS_RECORD_NO_POST_20260709.md) /
  [RESEARCH_RUNBOOK_NO_POST.md](RESEARCH_RUNBOOK_NO_POST.md)。
- 直前フェーズ:
  **VOL_REGIME_CONDITIONAL_BREAKOUT_RETEST_NO_POST
  pre-registered regime-conditioned breakout scored / NOT_ROBUST reject / no-POST**。
  2026-07-08、事前登録契約 §3 を実装: 既存 redesign runner に **vol-regime entry gate** を追加
  （`_high_vol_regime_flags`=ATR(14)>直前200barのATR中央値の high-vol bar のみ・lookaheadなし・
  fail-closed。candidate=**BREAKOUT×EXIT_RIDE×HIGH**・単一variant・スキャンなし。frozen decision/engine無変更。
  `RedesignCandidate.vol_regime_mode`/`vol_regime_lookback`追加、`evaluate_under_standard_gate`に
  window_bars_resolutions/lead override追加）。**新規取得なし**・既存 USD_JPY M5 2.7年(198,129 bars)を
  primary、H1 7,828 bars を参照として標準 multi-resolution gate で **1回採点**（M5: wb=12000/16800・lead250、
  cost2.0×・0.5pip/side・leg別spread・sign-permutation・min_qual_windows4）。
  結果: **M5 median PF 0.77/0.70・全窓 base_pass 0.0・符号置換に届かず・robust_all=False → NO_ROBUST_EDGE**。
  regime control(full-period・base cost): 高volレジーム条件付けは breakout PF を改善する方向は確認
  （HIGH>OFF>LOW。H1: 0.80→0.94 明瞭・**M5: 0.70→0.71 僅少**）だが**いずれもPF<1（赤字）**で edge に届かない。
  H1参照も NOT_ROBUST（符号置換負け・2.0×コスト非生存・非unanimous）。
  status=**VOL_REGIME_CONDITIONAL_BREAKOUT_REJECTED**。program多重検定台帳の **null #1/3**
  （契約§5: さらにK=3連続 NOT_ROBUST で `RESEARCH_PLATFORM_CLOSEOUT` へ移行・operator判断で前倒し可）。
  残 longlist はデータ壁/標本過少で near-term困難(三角RV=mirage・月末=標本過少・cross-asset/event=外部データ)。
  full suite 7234 passed / ruff clean。実POST/broker write/private GET/credential/env read/取得=ゼロ、
  raw値/CSV commitなし。performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_VOL_REGIME_CONDITIONAL_BREAKOUT_RETEST_NO_POST_20260708.md](STRATEGY_VOL_REGIME_CONDITIONAL_BREAKOUT_RETEST_NO_POST_20260708.md)。
- 直前フェーズ:
  **STRATEGY_HYPOTHESIS_INVENTORY_AND_PREREGISTRATION_NO_POST
  rejection ledger + next-candidate pre-registration / docs-only / no fetch / no-POST**。
  2026-07-08、4連続棄却(M5 tech / H1 trend / SESSION / GOTOBI)を受け、(a)棄却インベントリ台帳、
  (b)候補longlist(tier付)、(c)program全体の多重検定台帳(~20 trial・単一指標超えでは合格とせず常に
  unanimous多対照×multi-resolutionを要求・**新pre-registered仮説がK=3連続NOT_ROBUSTなら
  RESEARCH_PLATFORM_CLOSEOUTへエスカレーション**)を凍結。bounded targeted research反映:
  三角relative-valueは**prior低に降格**(文献: 小口三角裁定は"mirage"・3 spread同時)、
  vol-regime条件付けは**高volでtrend改善のdocumented支持ありだがoverfit注意**。
  → **唯一の近接actionable候補として `VOL_REGIME_CONDITIONAL_BREAKOUT` を事前登録(凍結)**:
  base=frozen redesign BREAKOUT×EXIT_RIDE(単一・スキャン禁止)、regime=atr_like(14) >
  直前200bars trailing median の high-vol bar のみentry(閾値=median 50%tile を a priori固定・lookaheadなし)、
  対象=**既存 USD_JPY M5 2.7年(198,129 bars)＋H1(参照)・新規取得なし**、gate=明文化済み標準
  (multi-resolution・cost2.0×・0.5pip/side・leg別spread・sign-permutation・M5用window_bars 12000/16800・
  high-vol vs low-vol対照)、min-sample未達はINSUFFICIENT、trial+1。
  ①RV(FX_RELATIVE_VALUE_TRIANGULAR)は契約skeletonのみ・**BLOCKED**(他ペア供給未確認＋prior低・要取得承認)。
  次step候補=`VOL_REGIME_CONDITIONAL_BREAKOUT_RETEST_NO_POST`(**operator承認不要・既存データで1回採点**)。
  code/fetch/backtest再実行なし・raw値/CSV commitなし。performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_HYPOTHESIS_INVENTORY_AND_PREREGISTRATION_NO_POST_20260708.md](STRATEGY_HYPOTHESIS_INVENTORY_AND_PREREGISTRATION_NO_POST_20260708.md)。
- 直前フェーズ:
  **GOTOBI_MULTI_YEAR_M5_PUBLIC_GET_AND_RETEST_NO_POST
  frozen rule scored on ~2.7yr M5 / NOT_ROBUST reject / no-POST**。
  2026-07-08、事前登録契約に基づき §10コード改修（leg別spread/末日暦/曜日層化置換/3ブロック安定/
  最小標本≥90・ラベル）を実装（synthetic testのみ・既存16件は再採点せず）。operator承認のもと
  bounded プローブで **GMO public FX klines の M5履歴は~2023-11開始**（それ以前HTTP404。既存窓は
  export側ハードコードの運用選択でありAPI制約でなかった）と確定。承認取得で **USD_JPY M5 BID+ASK を
  2023-11-01〜2026-07-07 日次取得**（監査済み公開GET経路のみ・credential/env不使用・rate-limit安全・
  **repo外CSV・未commit**。198,129 bars・整合0異常）。凍結ルール（03:00→09:55 JST・leg別spread・
  0.5pip/side・2.0×・4対照+3ブロック）で **1回だけ採点**（post-OOS retuning なし）。
  結果: **gotobi 166件 PF≈1.00（2.0×コスト0.977・期待値NEGATIVE）／符号置換p90(1.175)に負け=方向優位なし／
  非ゴトー(0.79)・曜日層化置換(0.98)は僅かに上回るが breakeven未満・非方向・非持続／3ブロックPF(0.82,0.90,1.51)
  → `GOTOBI_EFFECT_NOT_ROBUST_REJECT`**。**前回M5 16件のPF5.38は小標本の偶然**と判明（166件でPF≈1.00へ収束・
  最小標本gateが偽陽性を却下）。status=**GOTOBI_REJECTED_ON_MULTI_YEAR_M5**。次はpost-OOS retuning禁止のため
  新規事前登録×新規/独立データ or 別仮説のみ。full suite 7230 passed / ruff clean。
  実POST/broker write/private GET/credential/env read=ゼロ、public GETは承認済み・raw値/CSV commitなし。
  performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_GOTOBI_MULTI_YEAR_M5_RETEST_NO_POST_20260708.md](STRATEGY_GOTOBI_MULTI_YEAR_M5_RETEST_NO_POST_20260708.md)。
- 直前フェーズ:
  **GOTOBI_FIX_DRIFT_PREREGISTRATION_NO_POST
  frozen pre-registration contract before long-M5 retest / docs-only / no fetch / no-POST**。
  2026-07-08、GOTOBI retest(長期M5 public GET)へ進む前に、後出し変更を封じる**事前登録契約**を
  凍結(docs のみ・コード/取得/backtest再実行なし)。固定内容: 対象=USD_JPY/M5 primary(M1後回し・
  H1除外reference-only)、entry03:00 JST/exit09:55 JST 単一primary(secondaryは{09:50,09:55}最大2点・
  全点合格 or p95・台帳記録)、ゴトー日={5,10,15,20,25,**末日**}+**日本銀行営業日前倒し**(祝日+年末年始)、
  約定=**leg別crossing-side spread**(short=exit足ask)+slippage0.5pip/side+**2.0×cost stress**、
  対照=**5種unanimous**(コスト込みPF>1&期待値非負/非ゴトー対照/符号置換p90/**曜日層化ラベル置換p90**/
  **3ブロック以上全てPF>1**)、最小標本=**gotobi≥90&各ブロック≥25**、合格=
  `RETEST_PASSED_CANDIDATE_FOR_PAPER_FORWARD`(paper-forward解錠のみ)、多重検定台帳+**post-OOS retuning禁止**。
  重要訂正: 既存H1(15ヶ月)/M5(3ヶ月)の窓は**API制約でなくexport側ハードコードの運用選択**
  (`export_h1.py`の`WINDOW_START_UTC=2025-04-01`)。→ **GMO public FX klinesの真の履歴深度は未確認**。
  retest前に取得可能最古日をoperator把握 or 承認済み小プローブで確定要。
  retest step(`GOTOBI_MULTI_YEAR_M5_PUBLIC_GET_AND_RETEST_NO_POST`)は本契約を基準に
  §10のコード改修(leg別spread/末日暦/曜日層化置換/Nブロック/最小標本&ラベル)を実装→fetch→1回採点。
  full suite 7228 passed / ruff clean(コード無変更)。実POST/broker write/private GET/credential/
  env read/取得ゼロ、raw値/CSV commitなし。performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_GOTOBI_FIX_DRIFT_PREREGISTRATION_NO_POST_20260708.md](STRATEGY_GOTOBI_FIX_DRIFT_PREREGISTRATION_NO_POST_20260708.md)。
  **次Stepはoperator承認待ち**(public GET・取得range・最古日確認。§11)。
- 直前フェーズ:
  **STRATEGY_GOTOBI_FIX_DRIFT_HYPOTHESIS_NO_POST
  mechanism-based gotobi/Tokyo-fixing hypothesis pre-registered & tested / NOT confirmed on current data / no-POST**。
  2026-07-08、Webディープリサーチで機構ベース仮説を選定し、最有力の **GOTOBI_FIX_DRIFT(仲値ドリフト)**
  を新モジュール `backend/app/services/gmo_strategy_gotobi.py` に実装。機構=日本輸入企業がゴトー日
  (末尾5/0)にドル建て決済 → 銀行が9:55 JST仲値へドル買い → USD/JPYが3:00 JSTから9:55へ上昇しやすい
  (arXiv 2301.13204 [GMO外貨ex共著]・NBER w22820)。**事前登録した固定ルール**: 実効ゴトー日
  (基準日{5,10,15,20,25,30}を**日本銀行営業日**へ前倒し・静的祝日表2025-04..2026-07)に
  **03:00 JSTロング→09:55 JST決済**(TP/SLなし・03:00に板無い日はスキップ)。**4対照**
  (非ゴトー日/日ラベル置換p90/符号置換p90/期間前後半)を **0.5pip/side slippage+2.0×コスト**下で判定。
  結果(既存local CSV read-only): **H1(15ヶ月・gotobi 80件)=どの出口(09:55/09:00/08:00)でもPF<1・
  不安定 → NOT_ROBUST**(H1はexit足が10:00 JSTに閉じ仲値後リバーサルを含む=粒度不適)。
  **M5(3ヶ月・gotobi 16件)=4対照全通過・PF高だが16<30件で INSUFFICIENT_GOTOBI_SAMPLE**
  (評価器が小標本の好成績を robust と認めず正しく保留)。**現データでは仲値効果を確認できない**。
  適正検定には**複数年 M5/M1 USD/JPY**(sub-hour現象ゆえH1不可・3ヶ月M5は件数不足)が必要で、
  取得は **operator承認public GET前提**。full suite 7228 passed / ruff clean。実POST/broker write/
  private GET/credential/env read/新規取得ゼロ、raw price/spread/PnL/CSV commitなし。
  performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_GOTOBI_FIX_DRIFT_HYPOTHESIS_NO_POST_20260708.md](STRATEGY_GOTOBI_FIX_DRIFT_HYPOTHESIS_NO_POST_20260708.md)。
- 直前フェーズ:
  **STRATEGY_GATE_CORRECTION_AND_SESSION_HYPOTHESIS_NO_POST
  residual fixes + new session hypothesis + multi-resolution gate / all NO_ROBUST_EDGE / no-POST**。
  2026-07-08、前回確認で残した2残課題を是正: (a) exit touch を純関数 `_exit_touch` に抽出し
  **SHORT側はASK=BID+bar_spreadで判定**（買い戻し約定の楽観を除去）、(b) fixed-cadence random に代え
  **sign-permutation directional benchmark**（戦略のentry bar/gating/cadence/exitをそのまま、方向のみ
  ランダム化）を主benchmark化し `beats_random` の判定源に。標準gateを `StandardEvaluationGate` として
  明文化。再確認で **window-count 依存の偽robust**（粗い窓でrobust・細かい窓でNOT_ROBUST）を発見し、
  **多解像度(1000/1400 bars)で unanimous robust 要件**を追加（`evaluate_under_standard_gate` →
  `MultiResolutionGateReport`）。テクニカル非依存の新仮説 **SESSION_MOMENTUM**（session-open
  continuation・JST時刻ベース・別バッテリー）も構築。**engineの売買ロジック/frozen signal engineは無変更**。
  H1(7,828 bars)結果: **テクニカル8候補も新仮説SESSION_MOMENTUMも robust_all_resolutions=False →
  NO_ROBUST_EDGE**。特に session×ride は raw median PF~1.2 だが 7窓解像度で **beats_dir=False**
  （sign-permutationが「方向優位の不在」を検出。fixed-cadence randomでは見逃していた）。
  full suite 7215 passed / ruff clean。実POST/broker write/private GET/credential/env read/新規取得ゼロ、
  raw price/spread/PnL/CSV commitなし。performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_GATE_CORRECTION_AND_SESSION_HYPOTHESIS_NO_POST_20260708.md](STRATEGY_GATE_CORRECTION_AND_SESSION_HYPOTHESIS_NO_POST_20260708.md)。
  次の実質前進は、**別の独立期間/別銘柄での同一gate再確認（operator承認public GET前提）** または
  **多要因/条件付き仮説の事前登録検証（多重検定予算管理下）**。live移行は robust edge 不在のため不可。
- 直前フェーズ:
  **STRATEGY_EXECUTION_REALISM_HARDENING_NO_POST
  H1 candidate REJECTED under realistic slippage / evaluation gaps fixed / no-POST**。
  2026-07-08、Fable5妥当性確認で判明した3つの過小評価（verdictのcost stressが実際は1.5×止まり・
  random benchmarkが単一path・スリッページ未モデル化）を是正した。3 runner（baseline/redesign/
  random）に後方互換 `slippage_price_per_side`（entry+exitの2側にadverse fill）を追加、
  verdict の cost stress を **max(cost_multipliers)=2.0× に修正**、random benchmark を
  **多seed(30)分布の p90 超え**に修正。**engineの売買ロジックは無変更**。
  H1 slippage感度: 0.0pip→median PF 1.060(5/8)、0.3pip→1.024(5/8)、**0.5pip→1.001(4/8)**、
  0.8pip→0.968(3/8)。**break-evenは~0.5 pip/side**。是正後フルバッテリー（slippage 0.5pip/side・
  cost 2.0×・random p90）では **全8候補 NOT_ROBUST_REJECT → NO_ROBUST_EDGE**。
  前回 ROBUST とされた TREND_CONTINUATION×ride は **現実的摩擦で robust バーを割る**（見かけの
  優位は execution friction のマージン内だった）。**偽edgeを paper-forward前に却下できた**のが本質的成果。
  実POST/broker write/private GET/credential/env read/新規取得はゼロ、raw price/spread/PnL/CSV commitなし。
  performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_EXECUTION_REALISM_HARDENING_NO_POST_20260708.md](STRATEGY_EXECUTION_REALISM_HARDENING_NO_POST_20260708.md)。
  次の実質前進は、**複数年/複数期間の独立OOS（本slippage前提で再確認）** または
  **テクニカル以外のシグナル源への仮説刷新**。
- 直前フェーズ:
  **EXPAND_HISTORICAL_DATA_RANGE_OR_TIMEFRAME_NO_POST
  H1 long-range data + hardening re-applied / one ROBUST candidate for paper-forward / no-POST**。
  2026-07-08、より高い時間足・長期でデータを揃え直した。GMO Public klines を **public GET
  のみ**で USD_JPY **H1・7,828 bars・~15ヶ月（2025-04-01〜2026-07-07）** 取得（H4は公開API非対応・
  認証/POSTなし・CSVはrepo外・非commit）。adapterで official candidate 判定後、
  既存 `gmo_strategy_evaluation_hardening.py` を**コード変更なしで再適用**（8窓×900bars・
  cost 1.0/1.5/2.0・random benchmark）。M5では全滅（NO_ROBUST_EDGE）だったが、
  **H1では TREND_CONTINUATION__EXIT_RIDE_ATR が ROBUST_CANDIDATE_FOR_PAPER_FORWARD**
  （base pass 0.625=5/8窓・median PF 1.06・1.5×コストでも0.625・beats_random）→ overall
  ROBUST_CANDIDATE_FOUND。**spread比が下がると trend continuation が成立し得る**仮説と整合。
  ただし **edgeの証明ではない**（8候補中1つ通過＝多重検定リスク、pass率0.625はmarginal、
  exit設定に敏感、単一期間・bar-level spread近似）。performance_proof_status=false /
  live_ready=false は維持。実POST/broker write/private GET/credential/env readはゼロ、
  raw price/spread/PnL/CSV commitなし。
  記録: [STRATEGY_TIMEFRAME_EXPANSION_H1_HARDENING_NO_POST_20260708.md](STRATEGY_TIMEFRAME_EXPANSION_H1_HARDENING_NO_POST_20260708.md)。
  次の推奨Stepは **PAPER_FORWARD_TEST_DESIGN_NO_POST**（当該候補を対象・**多重検定補正と
  多窓/複数期間の再現確認をセット**で）＋さらなるデータ拡張。
- 直前フェーズ:
  **STRATEGY_EVALUATION_HARDENING_NO_POST
  walk-forward + cost-sensitivity + random benchmark / NO ROBUST EDGE / no-POST**。
  2026-07-08、単発 freeze→OOS が偽陽性（redesignのmean-reversion/ride候補）を出した反省から、
  評価の信頼度を上げる基盤 `gmo_strategy_evaluation_hardening.py` を追加した。
  ①ウォークフォワード（rolling OOS 複数窓でpass率を測る）②コスト感度（spread cost
  乗数 1.0/1.5/2.0）③決定論的LCGランダムentryベンチマーク（`random`モジュール不使用）
  ④候補別の総合頑健性判定（walk-forward robust ∧ cost robust ∧ beats random）。
  engineは無変更・後方互換な `spread_cost_multiplier`（既定1.0）を両runnerに追加。
  実データ（USD_JPY M5・9窓×2,000bars）結果: **全8候補が NOT_ROBUST/INSUFFICIENT、
  overall = NO_ROBUST_EDGE_ACROSS_WALK_FORWARD_AND_COST_STRESS**。前回freeze候補も
  9窓中4窓しかPF>1を満たさず（median PF 0.91<1）、**新枠組みが偽陽性を却下**。多くの候補は
  beats_random=yes（entryは限界的情報を持つ）だが spread込みで持続PF>1に届かず。baseline
  pass率も0.22。**M5テクニカルはコスト差引後に持続edgeなしが再現性を持って確認**。
  実POST/broker write/real HTTP/download/private GET/credential/env readはゼロ、
  raw price/spread/PnL/CSV commitなし。performance_proof_status=false / live_ready=false。
  記録: [STRATEGY_EVALUATION_HARDENING_NO_POST_20260708.md](STRATEGY_EVALUATION_HARDENING_NO_POST_20260708.md)、
  [STRATEGY_EVALUATION_HARDENING_REPORT_NO_POST_20260708.md](STRATEGY_EVALUATION_HARDENING_REPORT_NO_POST_20260708.md)。
  次の実質前進は **EXPAND_HISTORICAL_DATA_RANGE_OR_TIMEFRAME_NO_POST**（長期・複数期間・
  別時間足で本hardening再適用）または **STRATEGY_HYPOTHESIS_REBUILD_NO_POST**。
- 直前フェーズ:
  **STRATEGY_RULE_REDESIGN_NO_POST
  redesigned rules compared / candidate frozen from validation / OOS REJECTED / no-POST**。
  2026-07-08、baseline失敗要因（即時opposite exit依存＋過大な固定TP/SL）に対し、
  `gmo_strategy_redesign.py` を追加（**deterministic signal engine無変更**・backtest-only
  の別runner）。算術feature converter（SMA-trend/momentum/breakout/mean-reversion/ATR。
  ATRはexit距離算定にのみ内部使用・raw値非露出）と **ATR相対exit** を導入し、4 entry
  family × 2 exit config = 8候補を train/validation のみで比較（chronological・OOS温存・
  parameter_search_count=8）。**MEAN_REVERSION_RANGE__EXIT_RIDE_ATR** が validation
  PF 1.107・expectancy POSITIVE・maxcl 10(<baseline19) で選定基準を満たし freeze。
  **freeze後のOOS 1回で PF 0.643・expectancy NEGATIVE に劣化 →
  OOS_DEGRADED_REJECT_CANDIDATE**。OOS exitは SL119/TP37/opposite6 で、ATR相対exitが
  実際に機能（baselineのopposite依存3,168からの構造改善）したが、netでは持続edgeなし。
  OOSを見た後の再調整なし。**performance_proof_status=false / live_ready=false**。
  実POST/broker write/real HTTP/download/private GET/credential/env readはゼロ、
  raw price/spread/PnL/CSV commitなし。
  記録: [STRATEGY_RULE_REDESIGN_NO_POST_20260708.md](STRATEGY_RULE_REDESIGN_NO_POST_20260708.md)、
  [STRATEGY_RULE_REDESIGN_REPORT_NO_POST_20260708.md](STRATEGY_RULE_REDESIGN_REPORT_NO_POST_20260708.md)。
  次の推奨Stepは **STRATEGY_HYPOTHESIS_REBUILD_NO_POST** または
  EXPAND_HISTORICAL_DATA_RANGE_OR_TIMEFRAME_NO_POST。
- 直前フェーズ:
  **STRATEGY_BACKTEST_RESULT_REVIEW_RULE_IMPROVEMENT_AND_REVALIDATION_NO_POST
  baseline reviewed / candidates compared on train+validation / NO_CANDIDATE_SELECTED / no-POST**。
  2026-07-08、初回real-data backtestのbaselineを review（failure factor分類）し、
  改善候補を bounded に比較した。`gmo_strategy_rule_revalidation.py` を追加
  （backtest-level knob: `run_synthetic_backtest` に `entry_momentum_strict` /
  `opposite_signal_debounce_bars` を後方互換で追加。**deterministic signal engine
  無変更**）。候補: A entry strict / B opposite debounce / C tight exit / A+B combo。
  train/validation比較（chronological・OOS温存・parameter_search_count=4）の結果、
  **NO_CANDIDATE_SELECTED**（PF改善のB/A+Bは max_consecutive_losses 悪化で保守guardが
  却下、A/Cは改善不足、全候補expectancy NEGATIVE）。**freeze候補なし→OOS未実行
  （OOS_NOT_RUN_NO_CANDIDATE）**。ラベル精緻化: report に
  `REPORT_OPERATOR_LOCAL_CSV_*`、metrics に
  `OVERFITTING_RISK_UNKNOWN_SINGLE_SAMPLE_REAL_DATA`、engineの
  `real_data_used`/`synthetic_fixture_only` を dataset由来で正確化。
  **performance_proof_status=false / live_ready=false**。実POST/broker write/real HTTP/
  download/private GET/credential/env readはゼロ、raw price/spread/PnL/CSV commitなし。
  記録: [STRATEGY_BACKTEST_RESULT_REVIEW_NO_POST_20260708.md](STRATEGY_BACKTEST_RESULT_REVIEW_NO_POST_20260708.md)、
  ...
  結論: 現engine+候補では spread込みで edge を確立できず、incremental tweakは不十分。
  次の推奨Stepは **STRATEGY_RULE_REDESIGN_NO_POST**（trend/momentum flip依存からの
  再設計）、必要なら EXPAND_HISTORICAL_DATA_RANGE_OR_TIMEFRAME_NO_POST。
- 直前フェーズ:
  **HISTORICAL_DATA_IMPORT_DRY_RUN_AND_BACKTEST_WITH_OPERATOR_CSV_NO_POST
  operator CSV dry-run passed / initial real-data backtest completed UNPROVEN / no-POST**。
  2026-07-08、operator提供のGMO public kline BID/ASK local CSV（USD_JPY M5・
  19,992 bars・JST 2026-04-01〜2026-07-07）をread-only validate→
  `CSV_INTAKE_READY_OFFICIAL_EVALUATION`（spread込み・bar-level近似ラベル付き・
  BID/ASK完全alignment）。synthetic-only制約は最小変更で解除:
  `BacktestDataset.validated_operator_local_csv`（adapter validation通過時のみtrue）+
  status `DATASET_VALID_OPERATOR_LOCAL_CSV` を追加し、**未validated real/remote/
  forged datasetは引き続きfail-closed**（既存synthetic testは無変更で通過）。
  初回backtest（既存engine・候補policy CANDIDATE_MEDIUM_BALANCED・chronological
  split・OOS温存）は完了したが **profit factor≈0.83 / expectancy NEGATIVE で
  baselineに edge の根拠なし**。**これは性能証明ではなく初回探索の観察**
  （performance_proof_status=false / live_ready=false）。
  実POST/broker write/real HTTP/download/private GET/credential/env readはゼロ。
  raw OHLC/spread/PnL行・CSV内容・CSV commitはなし。
  dry-run記録: [HISTORICAL_DATA_IMPORT_DRY_RUN_WITH_OPERATOR_CSV_NO_POST_20260708.md](HISTORICAL_DATA_IMPORT_DRY_RUN_WITH_OPERATOR_CSV_NO_POST_20260708.md)、
  backtest記録: [STRATEGY_BACKTEST_WITH_OPERATOR_CSV_INITIAL_NO_POST_20260708.md](STRATEGY_BACKTEST_WITH_OPERATOR_CSV_INITIAL_NO_POST_20260708.md)。
  既知のラベル限界: metrics/report module の一部ラベルがreal dataでも "SYNTHETIC"/
  "NO_REAL_DATA" 語を保持（保守的に未証明側へ倒す固定ラベル・危険側ではない）。
  次の推奨Stepは **STRATEGY_BACKTEST_RESULT_REVIEW_NO_POST**（結果レビュー・
  engine/policy改善候補・real-dataラベル精緻化）。
- 直前フェーズ:
  **HISTORICAL_DATA_IMPORT_ADAPTER_NO_POST
  local CSV import adapter implemented / synthetic fixture verified / no-POST no-download**。
  2026-07-08、`gmo_historical_data_import_adapter.py` を追加した。
  local file only（path未提供=`DATA_ADAPTER_NOT_CONFIGURED`、URL/remote scheme/
  directory/非csvは**読取前block**、auto-discoveryなし）。combined CSV mode
  （spread列またはbid/ask列）と **BID/ASK pair mode**（timestamp/行数完全一致必須・
  bar-level spread導出・`SPREAD_FROM_BID_ASK_BAR_APPROXIMATION`+
  `NOT_TICK_LEVEL_SPREAD` label必須付与）を実装。禁止列（ID/credential/raw
  response系・表記ゆれ正規化検出）は値を読まずにblock。timestampはUTC epoch/
  TZ明示ISOのみ・単調・重複なし。OHLC/symbol/timeframe/source_label検証。
  sessionはJST policy（5:00-8:59=BLOCKED）から導出。出力は既存 `BacktestDataset`
  へ接続し `validate_backtest_dataset` 通過をテスト固定。spreadなし+official要求=
  `CSV_INTAKE_BLOCKED_MISSING_SPREAD`、OHLCのみ=REFERENCE_ONLY。
  **実CSV pathはNOT_PROVIDEDのため実データは一切未読**（tests 36件はtmp_path内
  synthetic CSVのみ）。real CSV dry-run手順は
  [HISTORICAL_DATA_IMPORT_DRY_RUN_INSTRUCTIONS_NO_POST.md](HISTORICAL_DATA_IMPORT_DRY_RUN_INSTRUCTIONS_NO_POST.md)、
  実装記録は [HISTORICAL_DATA_IMPORT_ADAPTER_NO_POST.md](HISTORICAL_DATA_IMPORT_ADAPTER_NO_POST.md)。
  performance_proof_status=false / live_ready=false。
  次の推奨Stepは、operator CSV準備済みなら
  **HISTORICAL_DATA_IMPORT_DRY_RUN_WITH_OPERATOR_CSV_NO_POST**、未準備なら
  **OPERATOR_PREPARE_GMO_PUBLIC_KLINE_BID_ASK_LOCAL_CSV**。
- 直前フェーズ:
  **HISTORICAL_DATA_SOURCE_SELECTION_NO_POST
  historical data source selected / local CSV intake spec fixed / no-POST no-download**。
  2026-07-08、実データ取得なしでソース選定を完了した。
  `gmo_historical_data_source_selection.py` が候補分類をfail-closedで固定:
  credential/API/real HTTP必須source=BLOCKED、synthetic=DEVELOPMENT_ONLY、
  OHLCのみ=REFERENCE_ONLY、OHLC+spread(またはbid/ask)+TZ+session=OFFICIAL候補。
  **推奨route Primary = `GMO_PUBLIC_KLINES_BID_ASK_LOCAL_CSV`**
  （既存 `fetch_gmo_public_market_data.py` はBID/ASK両klines対応・認証不要・
  UTC epoch。BID+ASK両series exportでbar-level spread導出可 —
  `OFFICIAL_CONDITION_SPREAD_FROM_BID_ASK_BAR_APPROXIMATION` 条件付き。
  **取得実行はoperator承認の別Step**）。Secondary=GMO会員画面手動export
  （spread列はoperator確認）。CSV受入契約（必須/任意/禁止columns・validation
  rules・intake categories）は
  [HISTORICAL_DATA_LOCAL_CSV_INTAKE_SPEC_NO_POST.md](HISTORICAL_DATA_LOCAL_CSV_INTAKE_SPEC_NO_POST.md)、
  選定記録は [HISTORICAL_DATA_SOURCE_SELECTION_NO_POST.md](HISTORICAL_DATA_SOURCE_SELECTION_NO_POST.md)。
  operator準備物: symbol/timeframe/date range確定、public kline export承認、
  local CSV path提供、利用条件確認。performance_proof_status=false / live_ready=false。
  次の推奨Stepは **HISTORICAL_DATA_IMPORT_ADAPTER_NO_POST**。
- 直前フェーズ:
  **STRATEGY_BACKTEST_AND_EVALUATION_READINESS_NO_POST
  backtest dataset requirements + schema + engine skeleton + metrics + report /
  synthetic fixtures only / no-POST**。
  2026-07-08、実データ投入前の土台を4 module（標準ライブラリのみ）で追加した:
  `gmo_strategy_backtest_dataset.py`（schema・fail-closed validation・
  chronological split + leakage assert・adapter設計。CSV/broker export adapterは
  `DATA_ADAPTER_NOT_CONFIGURED` でfail-closed・download surfaceなし）、
  `gmo_strategy_backtest_engine.py`（candle→safe label変換境界＝算術のみ・
  既存signal engineの時系列適用・paper entry/exit skeleton・TP/SL/max hold
  **candidate** policy＝officially_adopted構築拒否）、
  `gmo_strategy_backtest_metrics.py`（21指標pipeline・最小30 trade未満は評価保留・
  spread excludedはofficial評価block・overfitting既定=RISK_UNKNOWN_NO_REAL_DATA）、
  `gmo_strategy_backtest_report.py`（14 section固定・集計のみ・過大表現は
  renderer例外・spread excluded=REFERENCE_ONLY）。synthetic fixture検証は
  tests 58件 green（TP/SL/max hold/EOW exit・block系・invalid dataset系・
  split no-leakage・spread included/excluded区別）。
  **実データ取得ゼロ・synthetic結果はperformance proofではない
  （performance_proof_status=false / live_ready=false）**。
  dataset要件: [STRATEGY_BACKTEST_DATASET_REQUIREMENTS_NO_POST.md](STRATEGY_BACKTEST_DATASET_REQUIREMENTS_NO_POST.md)、
  report形式・指標・overfitting防止: [STRATEGY_BACKTEST_REPORT_FORMAT_NO_POST.md](STRATEGY_BACKTEST_REPORT_FORMAT_NO_POST.md)、
  evidence: [STRATEGY_BACKTEST_AND_EVALUATION_READINESS_NO_POST_20260708.md](STRATEGY_BACKTEST_AND_EVALUATION_READINESS_NO_POST_20260708.md)。
  次の推奨Stepは **HISTORICAL_DATA_SOURCE_SELECTION_NO_POST**（operator判断）→
  **HISTORICAL_DATA_IMPORT_ADAPTER_NO_POST** → 実データ投入後に
  **STRATEGY_BACKTEST_WITH_REAL_DATA_NO_POST**。
- 直前フェーズ:
  **STRATEGY_SIGNAL_ENGINE_SUPERVISED_EVALUATION_NO_POST
  strategy signal engine formalized + supervised behavior evaluation PASSED / no-POST**。
  2026-07-08、暫定signal logicを `gmo_strategy_signal_engine.py` として明文化した
  （deterministic・rule-based・safe-label-only・固定table lookup。LLM自由判断は
  売買判断に不使用。environment gates → position context → trend×momentum の順で
  評価し、UNKNOWN/CONFLICT/未定義はすべてfail-closed。ONE_POSITION contextは
  entry preview禁止=settlement context only。出力はAUTO_PREVIEW_SIGNAL_*のみで
  operator signalでもpermissionでもない）。rulebookは
  [STRATEGY_SIGNAL_ENGINE_RULEBOOK_NO_POST.md](STRATEGY_SIGNAL_ENGINE_RULEBOOK_NO_POST.md)。
  `gmo_strategy_signal_supervised_evaluation.py` の挙動評価は
  **BEHAVIOR_PASSED: families 17/17・grid 52行・rule coverage complete・
  fail-closed 41行・HOLD発注0・operator review placeholder=NOT_PROVIDED（AI非記入）**。
  evidence: [STRATEGY_SIGNAL_ENGINE_SUPERVISED_EVALUATION_NO_POST_20260708.md](STRATEGY_SIGNAL_ENGINE_SUPERVISED_EVALUATION_NO_POST_20260708.md)。
  **strategy品質・収益性は未証明（performance proof status=false）**。
  actual POST=0 / broker write・real HTTP・private GET・credential・env read なし。
  unattended live remains unsupported / unattended full auto completed=false。
  次の推奨Stepは operator 判断で **STRATEGY_BACKTEST_DATASET_REQUIREMENTS_NO_POST**、
  **STRATEGY_SIGNAL_OPERATOR_REVIEW_WORKFLOW_NO_POST**、または
  **LIVE_RUNTIME_SAFE_LABEL_ADAPTER_DESIGN_NO_POST**。
- 直前フェーズ:
  **STEP_6G_PC_OX_AA_LIMITED_UNATTENDED_PAPER_SOAK_NO_POST_C
  limited unattended paper soak executed / PASSED / no-POST**。
  2026-07-08、`gmo_limited_unattended_paper_soak.py` を追加し、限定無人paper soakを
  即時synthetic実行（sleep/daemon/cron/schedulerなし）で完了した。結果は
  **LIMITED_PAPER_SOAK_PASSED: 55 synthetic cycles / 55一致 / entry・settlement
  各最大1 attempt invariant成立 / no-retry invariant成立 / duplicate 4件すべてblock /
  guard halt 8種はattempt前 / fake notifier外部送信ゼロ / real POST count=0**。
  preview signal は operator signal に変換されず、permission化ゼロ。planは
  固定safe labelで `validate_limited_paper_soak_plan` によりfail-closed検証。
  evidence: [LIMITED_UNATTENDED_PAPER_SOAK_NO_POST_20260708.md](LIMITED_UNATTENDED_PAPER_SOAK_NO_POST_20260708.md)。
  **paper/synthetic結果でありlive性能証明ではない。unattended live remains
  unsupported / unattended full auto completed=false（不変）**。
  次の推奨Stepは operator 判断で
  **STRATEGY_SIGNAL_ENGINE_SUPERVISED_EVALUATION_NO_POST**（strategy品質）または
  **LIVE_RUNTIME_SAFE_LABEL_ADAPTER_DESIGN_NO_POST**（live監視統合設計）。
- 直前フェーズ:
  **UNATTENDED_MONITORING_GUARD_AND_PAPER_SOAK_READINESS_NO_POST
  monitoring guard + paper cycle state machine/lock/ledger + fake notifier +
  paper soak readiness runner / no-POST**。
  unattended live の前提となる監視guard層を paper/synthetic 限定で追加した。
  `gmo_unattended_monitoring_guard.py`（fail-closed・UNKNOWN即halt・固定優先順位、
  kill switch / market / ticker / spread / position mismatch / active-pending /
  max hold / max loss / failure limit / real transport / state consistency。
  GUARD_PASSはactual permissionではない）、
  `gmo_unattended_cycle_state_machine.py`（legal transition固定・illegalはsafe halt・
  paper lock必須・safe attempt ledgerで entry/settlement 各最大1回・duplicate即halt）、
  `gmo_unattended_fake_notifier.py`（in-memory collectorのみ・external_send=false固定）、
  `gmo_unattended_paper_soak_runner.py`（synthetic 21 scenario suite・常駐なし。
  結果 `PAPER_SOAK_READINESS_PASSED` 21/21）。actual POST / broker write / real HTTP /
  private GET / credential / `.env` はゼロ。
  **unattended live remains unsupported / unattended full auto completed=false（不変）**。
  詳細は [UNATTENDED_MONITORING_GUARD_AND_PAPER_SOAK_READINESS_NO_POST.md](UNATTENDED_MONITORING_GUARD_AND_PAPER_SOAK_READINESS_NO_POST.md)、
  運用手順は [UNATTENDED_MONITORING_RUNBOOK_NO_POST.md](UNATTENDED_MONITORING_RUNBOOK_NO_POST.md)。
  次の推奨Stepは **LIMITED_UNATTENDED_PAPER_SOAK_NO_POST**（paper soakの限定実行）。
- 直前フェーズ:
  **STEP_6G_PC_OX_Z_OPERATOR_GATED_CYCLE_CLOSEOUT_TO_SUPERVISED_AUTO_LIVE_PREVIEW_NO_POST_C
  operator-gated live cycle closeout + unattended gap assessment + paper auto cycle runner +
  supervised auto live preview / no-POST**。
  2026-07-08、operator-gated の live 1サイクル（entry POST 1回
  `RESULT_ACCEPTED_SANITIZED` → read-only `ONE_POSITION_OPEN/COUNT_ONE` →
  official settlement POST 1回 `SETTLEMENT_RESULT_ACCEPTED_SANITIZED` →
  read-only `NO_POSITION/COUNT_ZERO`）が完了した。各POSTは operator current-turn
  exact confirmation（banking不可）を伴い、retry/repost/second POST・generic close・
  raw/ID/value/credential露出はゼロ。**operator定義で Level 5 operator-gated cycle
  completed=true、ただし unattended full auto completed=false（不変）**。
  safe記録は [LIVE_OPERATOR_GATED_CYCLE_CLOSEOUT_20260708_NO_POST.md](LIVE_OPERATOR_GATED_CYCLE_CLOSEOUT_20260708_NO_POST.md)、
  人間が担った判断の分類と無人化までの残gapは
  [UNATTENDED_GAP_ASSESSMENT_NO_POST.md](UNATTENDED_GAP_ASSESSMENT_NO_POST.md)。
  entry側実行基盤は `gmo_live_actual_entry_execution_boundary/sender`、settlement側は
  `gmo_live_official_settlement_preflight/execution_boundary/sender`（いずれもfail-closed・
  one-shot・operator gate必須）。加えて no-POST の自動化検証層として
  `backend/app/services/gmo_paper_auto_cycle_runner.py`（fake transport限定の
  paper cycle runner・operator label拒否・AUTO_PREVIEW_SIGNAL_*のみ受理）と
  `backend/app/services/gmo_supervised_auto_live_preview.py`（自動提案の
  preview package・permissionではない・operator confirmation必須を明示）を追加した。
  詳細は [PAPER_AUTO_CYCLE_RUNNER_NO_POST.md](PAPER_AUTO_CYCLE_RUNNER_NO_POST.md) と
  [SUPERVISED_AUTO_LIVE_PREVIEW_NO_POST.md](SUPERVISED_AUTO_LIVE_PREVIEW_NO_POST.md)。
  次に実POSTへ進む場合は、fresh gate 一式と operator current-turn confirmation が
  従来どおり必須。unattended live は当面 unsupported。
- 直前フェーズ:
