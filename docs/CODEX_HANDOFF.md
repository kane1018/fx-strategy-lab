...
- 現在のフェーズ:
  **Step 6G-PC-OX-R-MANUAL-FLATTEN-THEN-RUNTIME-FLAT-RECONCILIATION-C manual flatten runtime flat reconciliation / read-only / CASE 1**。
  ...
  `has_multiple_positions=true` だった。operator manual flatten後の今回Stepでは
  operator safe booleanがすべて期待値どおりで、runtime safe readは
  `position_status=NO_POSITION`、`position_count_safe=0`、
  `has_multiple_positions=false`。したがって
  ...
  今回
  `backend/app/live_verification/live_order_real_manual_flatten_runtime_flat_reconciliation_controlled.py` と
  `backend/app/tests/test_live_verification_live_order_real_manual_flatten_runtime_flat_reconciliation_controlled.py`、
  [STEP6G_MANUAL_FLATTEN_THEN_RUNTIME_FLAT_RECONCILIATION.md](STEP6G_MANUAL_FLATTEN_THEN_RUNTIME_FLAT_RECONCILIATION.md)
  を追加した。GMO FX公式manual/rulesをauthoritative sourceとして扱い、generic opposite orderは
  settlement primitiveではない方針を維持する。
  ...
  次の推奨Stepは **Step 6G-PC-OX-R-GMO-OFFICIAL-SETTLEMENT-ROUTE-REVIEW-C**。
- 直前フェーズ:
