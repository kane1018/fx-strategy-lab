...
- 現在のフェーズ: **Phase 2E-2 shadow session統合前の安全接続設計完了**。Phase 2E-1Hでは`app/shadow/`内の
  ...
  virtual result相関、summary互換、統合test方針を整理した。既存shadow sessionには未統合で、
  Phase 2E-2実装へは別タスクの明示承認まで進まない。設計は
