...
- 現在のフェーズ:
  **Step 6G-PC-OX-R-POSITION-READ-ONLY-ROUTE-WIRING-C position read-only route wiring / no actual POST / no close POST**。
  Level 5 fast-track MVP foundationの後続として、
  `backend/app/live_verification/live_order_real_position_read_only_controlled.py` を追加し、safe status/countだけの
  position read-only routeを実装した。`NO_POSITION` はentry planningのみ許可、`ONE_POSITION_OPEN` はclose planningのみ
  許可、multiple/unknown/source missing/raw exposure/ID exposure/value exposure/credential exposureはfail-closedでentryと
  close planningを止める。`live_order_real_step6g_level5_fast_mvp_controlled.py` はこのroute resultを既存のposition
  status、signal、cycle、close foundationへ接続できる。実sourceはまだ未接続で、defaultは
  `SOURCE_MISSING_BLOCKED`。actual HTTP POST、close POST、retry/repost、second POST、ledger update、receipt handoff、
  raw request/response、broker/API response、account/order/transaction/position ID、credential/signature/header値は扱わない。
  詳細は [STEP6G_POSITION_READ_ONLY_CONTROLLED.md](STEP6G_POSITION_READ_ONLY_CONTROLLED.md)。
