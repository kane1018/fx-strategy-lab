...
- 現在のフェーズ: **Step 6G-PC-OX-R-ONE-SHOT-POST-REAL-TRANSPORT-BINDING-C controlled real transport binding implementation / no actual HTTP POST**。
  ...
  **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-GATE-RETRY-2**。次Stepでも最初からPOSTしてはならず、
  safe preview提示後、このCodexセッション内の新しいPOST-specific confirmationを取得してから、条件が揃う場合だけ
  最大1回のHTTP POSTを検討する。ledger/receipt/retry/repostは引き続き分離必須。
