...
- 現在のフェーズ:
  **Step 6G-PC-OX-R-CLOSE-ORDER-EXECUTION-ROUTE-IMPLEMENTATION-NO-POST-C close execution route foundation / concrete close side / no actual close POST / CASE 1**。
  前回 close execution gate はCASE 2で、`OPPOSITE_OF_SAFE_POSITION_SIDE` のままでは
  executable previewへ進めず、close-specific approved primitive contractも未整備だった。
  今回 `backend/app/live_verification/live_order_real_close_order_execution_route_controlled.py` と
  `backend/app/tests/test_live_verification_live_order_real_close_order_execution_route_controlled.py`、
  [STEP6G_CLOSE_ORDER_EXECUTION_ROUTE_CONTROLLED.md](STEP6G_CLOSE_ORDER_EXECUTION_ROUTE_CONTROLLED.md)
  を追加した。safe inputだけから `BUY -> SELL` / `SELL -> BUY` を具体化し、
  placeholder side、unknown side、side mismatch、non-ONE_POSITION_OPEN、unsafe exposureをblockする。
  guarded generic primitiveは exact-one-position guard、concrete opposite side、100 units、`MARKET`
  が揃う場合だけ `approved_close_post_primitive_ready=true` にできる。primitive invocationは
  deferredのままで、`actual_close_post_allowed_now=false`、actual close POST / entry POST /
  retry/repost / second close POST / ledger / receipt / raw/ID/value exposure は未実行。
  Level 5 foundation now carries `close_execution_route`; ready pathは
  `CLOSE_EXECUTION_GATE_READY_NO_POST` までで、`CLOSE_SENT` 以降には進まない。
  次の推奨Stepは
