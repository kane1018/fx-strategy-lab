...
- 現在のフェーズ: **Step 6E-CP Credential presence readiness完了 / no API / no POST**。
  ...
  Step 6E-CPでは実API、read-only API、public API、Private API、broker、order endpoint、
  `live_order_once`、HTTP POST、実注文、raw request/response表示・保存、
  headers/signature/credentials値表示・保存、ledger操作を行っていない。
  runtimeのPRESENT/MISSING結果はGitに保存しない。詳細は
