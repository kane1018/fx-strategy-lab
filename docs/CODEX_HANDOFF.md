...
- 現在のフェーズ: **Step 6G-PC-OX-R-LEDGER-FREE-POST-ONLY-SOURCE-FACTORY-C ledger-free POST-only source factory / no actual HTTP POST**。
  SEALED-CREDENTIAL-SIGNING-PROVIDER-C CASE 1（sealed credential/signing/headers provider foundation実装済み）を受けて、
  `backend/app/live_verification/live_order_real_one_shot_post_ledger_free_source_factory_controlled.py` と
  ...
  real header builder、ledger writer、receipt handoffをimport/callしない。testsではfake/monkeypatch delegateだけを使い、
  factory import/construct/summary/source constructionがPOSTしないこと、missing prerequisitesをfail-closedにすること、
  accepted/rejected/failed/timeout/unknown/unavailable mapping、exactly-once接続、no retry、no second POST、
  no ledger/receipt、raw/ID/value非露出を確認する。このStepではactual HTTP POST、order endpoint、`live_order_once`、
  POST-specific confirmation取得、ledger update、attempt counter persistence、actual receipt handoff、retry/repostには
  進んでいない。`approved_primitive_actual_source_available` はcurrent/default routeでtrue。次の推奨Stepは
  **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-GATE-RETRY-7**。次Stepでも最初からPOSTせず、sanitized preview提示後に
