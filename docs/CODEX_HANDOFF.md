...
- 現在のフェーズ: **Phase 2E-3.5 Public ticker bid/ask provenance連携監査完了（B判定）**。Phase 2E-3で
  GMO Public `/v1/ticker`由来bid/askだけを`REAL_PUBLIC_BID_ASK`として扱う`MarketSnapshot` validationを追加し、
  `--enable-shadow-risk`経路へ最小接続した実装をレビューした。valid Public tickerのみcandidate/risk/virtual resultへ
  進めること、kline-onlyがsynthetic spread rejectを維持すること、missing/invalid/stale/future/skew tickerがfail
  closedになること、raw responseを保存しないこと、summary/metadata互換を壊していないことを確認した。
  修正必須事項はなく、次はPhase 2E-4設計または実行指示作成へ進める。監査結果は
  [PHASE2E3_PUBLIC_TICKER_BID_ASK_AUDIT.md](PHASE2E3_PUBLIC_TICKER_BID_ASK_AUDIT.md)、実装詳細は
  [PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md](PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md)。今回もGMO Public実run、
