...
- 現在のフェーズ: **Phase 2E-3 Public ticker bid/ask provenance連携実装完了（local-only）**。GMO Public
  `/v1/ticker`由来bid/askだけを`REAL_PUBLIC_BID_ASK`として扱う`MarketSnapshot` validationを追加し、
  `--enable-shadow-risk`経路へ最小接続した。valid Public tickerのみcandidate/risk/virtual resultへ進め、
  kline-onlyはsynthetic spread rejectを維持し、missing/invalid/stale/future/skew tickerはfail closedに倒す。
  summary/metadataにはticker optional countsと`raw_response_saved=false`を追加し、legacy summary互換を維持した。
  実装詳細は [PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md](PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md)。
  今回もGMO Public実run、Private API、APIキー、broker、実注文、本番公開API追加は行っていない。
...
- 直近確認実績: backend 327 passed、`ruff check .` OK、production smoke 7 passed。
