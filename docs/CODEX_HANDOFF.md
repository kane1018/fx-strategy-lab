...
- 現在のフェーズ: **Phase 2E-3 Public ticker bid/ask provenance連携設計完了（docs-only）**。GMO Public
  `/v1/ticker`由来bid/askだけを`REAL_PUBLIC_BID_ASK`として扱う条件、kline-onlyをsynthetic spread rejectに
  倒す境界、timestamp/freshness/spread検証、raw response非保存、session no-network、fail closed、audit/summary
  metadata、実装時test方針を [PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md](PHASE2E3_PUBLIC_TICKER_BID_ASK_DESIGN.md)
  に整理した。今回は設計のみで、Public ticker bid/ask連携実装、GMO Public run、Private API、APIキー、broker、
  実注文、本番公開API追加は行っていない。Phase 2E-1Hでは`app/shadow/`内の
