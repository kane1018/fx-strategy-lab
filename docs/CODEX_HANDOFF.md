...
- 現在のフェーズ: **Step 6G-PC-OX-R-REAL-POST-DELEGATE-CONNECTION-C real POST delegate connection / no actual HTTP POST**。
  LEDGER-FREE-POST-ONLY-SOURCE-FACTORY-C CASE 1（ledger-free source factory実装済み、approved primitive actual source
  current/default route available）を受けて、
  `backend/app/live_verification/live_order_real_one_shot_post_real_delegate_controlled.py` と
  `backend/app/tests/test_live_verification_live_order_real_one_shot_post_real_delegate_controlled.py`、
  `docs/STEP6G_ONE_SHOT_POST_REAL_DELEGATE_CONTROLLED.md` を追加した。real delegate connectionは、
  `post_live_order_with_httpx` を実行しないfunction reference境界として扱い、ledger-free source factoryへdelegate supplied
  状態を渡す。current/default approved primitive actual source routeはdelegate-backedになり、
  `source_callable_unavailable_due_missing_delegate=false` となる。testsではfake/monkeypatch delegateだけを使い、
  delegate import/construct/summary/supply/source constructionがPOSTしないこと、accepted/rejected/failed/timeout/unknown/
  unavailable mapping、exactly-once接続、no retry、no second POST、no ledger/receipt、raw/ID/value非露出を確認する。
  このStepではactual HTTP POST、order endpoint、`live_order_once` execution、POST-specific confirmation取得、
  ledger update、attempt counter persistence、actual receipt handoff、retry/repostには進んでいない。次の推奨Stepは
  **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-GATE-RETRY-8**。次Stepでも最初からPOSTせず、sanitized preview提示後に
