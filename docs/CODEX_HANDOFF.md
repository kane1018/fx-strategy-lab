...
- 現在のフェーズ: **Step 3 独立した最終監査・preflight完了（今回実行はNO_GO） / Step 4は未実行**。
  ...
  **NO_GO**。HTTP POST、実注文、実資金検証、Private API書き込み、broker、`OrderRequest`、
  real order API client、本番公開API追加には進んでいない。Step 4へ進むには、別タスクで
  credential presenceがsetの環境からsanitized read-only preflightを再実行し、
