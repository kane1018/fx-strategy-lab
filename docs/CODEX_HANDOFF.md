...
- 現在のフェーズ:
  **Step 6G-PC-OX-R-POST-CLOSE-POSITION-CONFIRMATION-GATE-C post-close position confirmation / read-only / CASE 3**。
  直前 close execution gate with compatible executor はCASE 1で、safe summary上
  `close_http_post_executed=true`、`close_post_execution_count=1`、
  `close_sanitized_result_category=RESULT_ACCEPTED_SANITIZED`、
  `close_safe_reconciliation_status=RECONCILIATION_READY_NO_RECEIPT_HANDOFF`、
  close side `SELL`、symbol `USD_JPY`、units `100`、order type `MARKET`。
  retry/repost/second close=false、entry POST this Step=false、ledger/receipt=false、
  raw/ID/value exposure=falseとして扱う。
  今回
  `backend/app/live_verification/live_order_real_post_close_position_confirmation_gate_controlled.py` と
  `backend/app/tests/test_live_verification_live_order_real_post_close_position_confirmation_gate_controlled.py`、
  [STEP6G_POST_CLOSE_POSITION_CONFIRMATION_GATE.md](STEP6G_POST_CLOSE_POSITION_CONFIRMATION_GATE.md)
  を追加した。post-close runtime position safe readは1回のみ実行し、safe status/countは
  `position_status=MULTIPLE_POSITIONS_BLOCKED`、`position_count_safe=2`、
  `has_multiple_positions=true`。したがって
  `close_effect_confirmed_by_position=false`、
  ...
  close POST再実行、retry/repost、second close、entry POST、ledger update、receipt handoff、
  raw request/response、broker/API response、account/order/transaction/position ID、
  actual price/PnL、credential/signature/header値、`.env` は扱っていない。
...
- 直前フェーズ:
  **Step 6G-PC-OX-R-CLOSE-ORDER-ACTUAL-EXECUTOR-COMPATIBILITY-NO-POST-C close actual executor compatibility foundation / no actual close POST / CASE 1**。
  ...
  close-specific contextの場合だけ、close execution route foundation由来のsafe previewを入力にして
  `SELL` / `BUY` close sideを no-POST executor previewとして受け入れる。
  必須条件は exact-one-position guard、approved guarded generic close primitive、concrete close side、
  fixed `100` units、`MARKET`、retry/repost/second close=false、transport call count `0`。
  Level 5 foundation now carries `close_actual_executor_compatibility`; ready pathは
  `CLOSE_ACTUAL_EXECUTOR_COMPATIBILITY_READY_NO_POST` までで、`CLOSE_SENT` 以降には進まない。
