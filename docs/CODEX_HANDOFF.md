...
- 現在のフェーズ:
  **Step 6G-PC-OX-R-MANUAL-POSITION-RISK-CHECK-GATE-C manual position risk check / no POST / CASE 1**。
  ...
  今回
  `backend/app/live_verification/live_order_real_manual_position_risk_check_gate_controlled.py` と
  `backend/app/tests/test_live_verification_live_order_real_manual_position_risk_check_gate_controlled.py`、
  ...
  を追加した。GMO FX公式manual/rulesをauthoritative sourceとして扱い、買建玉と売建玉は
  coexistし得るためCodex route logicではnetしない。
  ...
  次の推奨Stepは **Step 6G-PC-OX-R-MANUAL-FLATTEN-THEN-RUNTIME-FLAT-RECONCILIATION-C**。
- 直前フェーズ:
