...
| 4 | USD/JPY取引画面または仕様ページ | 最小取引単位が公開仕様どおり10,000通貨か | `CONFIRMED_10000` or 実際の値カテゴリ | **未確認** |
| 5 | 外国為替FX API利用規約（ページ下部「各種規約」） | 自動売買・API利用に関する禁止事項・責任条項の有無 | `REVIEWED_NO_BLOCKING_CLAUSE` or 懸念点の要約 | **未確認**（免責条項は公開情報で部分確認済み） |
