...
Status: **SUPERSEDED_BY_OPERATOR_SELECTED_RELAXED_V4_NO_POST**

> 2026-07-15 update: 本書の「v3と同等の原子的安全性を必須とする」初期案は、operatorの
> 明示判断により採用しない。GMO向けの選定済みprofileと現在の正は
> [H11_V4_GMO_RELAXED_EXECUTION_PROFILE_NO_POST_20260715.md](H11_V4_GMO_RELAXED_EXECUTION_PROFILE_NO_POST_20260715.md)
> とする。v3自体は変更せず、actual activationも許可していない。
...
v4_execution_profile=H11_V4_GMO_MARKET_THEN_EXACT_OCO_NO_POST_V1
v4_config_hash=RUNNER_DERIVED_FROM_FROZEN_POLICY
actual_post=false
broker_read=false
broker_write=false
...
## 2. 当初案の非交渉条件（2026-07-15にsuperseded）
...
この段落は当初案の記録として残す。現在のv4は原子的保護を必須とせず、15秒以内の一時的無保護を
明示的に受容する別契約である。
