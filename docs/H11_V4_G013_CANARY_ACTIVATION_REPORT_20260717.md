...
## 1.11 2026-07-22 operation 30 administrator-context phase separation

- The generation bound to reviewed-files digest `sha256:58696d96c7b65d46b84a77a274e6f8ec6783ffd288c77a2aaf946b2bfb3608fc` and generation digest `sha256:e7e93bb53843b02d17003fe0f56da74fe2d40ad9a6ef33a68433663e1f1f9af2` completed operations 00 through 20, then retained only `30_host_kill.started`. The generation-specific host directory was created, but no risk state, coordinator state, or transport marker existed; broker GET/POST counts remained zero.
- The fixed administrator read allowed 120 seconds internally, while the GUI-capable Codex execution context ended at approximately 30 seconds. The durable state proves termination before the disposable KILL stage but cannot distinguish an unseen administrator dialog from execution-context termination, so the old operation remains unknown and no-retry.
- Corrective phase separation adds generation-bound operation `25_network_time`. It performs the one fixed read-only administrator command and records only sanitized On/Off evidence. Operation `30_host_kill` requires operation 25 passed and reuses only that safe predecessor result, so it performs no GUI authentication and can proceed directly to the disposable KILL proof.
- Both operations remain one-attempt and generation/day bound. No setting change command is accepted, and no credential, raw response, broker read/write, permit, risk limit, order intent, or prior marker is changed. External preparation restarts at operation 00; `actual_post_authorized=false` and broker POST count 0 remain unchanged.
