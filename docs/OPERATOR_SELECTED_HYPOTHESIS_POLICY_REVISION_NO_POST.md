...
2026-07-10 operator 確認済み。②選定仮説のみ**保留（PENDING_OPERATOR_SELECTION）**とし、
それ以外を確定して先行実装（§9）へ進む。
...
selected_hypothesis:
  registry_id_or_new:            PENDING_OPERATOR_SELECTION（保留。候補: H-06 / H-02 selected_despite_rejected=true）
  selected_despite_rejected:     （仮説確定時に記入）
  frozen_spec_doc:               （仮説確定時に記入）
...
- Stage 1 の実稼働（`STAGE1_PAPER_WIRING_STEP`）は §4 により選定仮説の凍結登録が完了するまで
  **着手不可**。`POLICY_ACTIVATION_STEP`（ACTIVE化・registry凡例追加・Evidence Charter処置・
  状態文書反映）は選定仮説に依存しないため先行実行可。
- 上記予算・停止基準の数値は仮説未確定のまま記入したものであり、仮説確定時に
