...
v3 actual activationの必須確認事項として残っている2点について、問い合わせ文面を用意した。

## 問い合わせ文面（コピー用）

```text
件名: 外国為替FX API（ifoOrder / IFDOCO）の仕様確認

外国為替FX APIのifoOrder（IFDOCO注文）について、公開ドキュメントで確認できなかった
点を2点お伺いします。
...
`broker-native pending expiry` / `actual partial-fill semantics` 行へ反映する。
