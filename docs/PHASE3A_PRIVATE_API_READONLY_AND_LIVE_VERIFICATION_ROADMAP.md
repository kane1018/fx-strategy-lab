...
- Phase 3C Live Verification Mode設計は完了。Live Verification Modeの定義、注文前read-onlyチェック、
  risk decision / candidate / order intent相関、order intent設計、kill switch / STOP / fail closed条件、
  実注文前後のチェックリスト、Phase 3Dへ進む条件をdocs-onlyで整理した。
- Phase 3C Live Verification Mode実装設計レビューは完了。Phase 3C-1 mocked core、Phase 3C-2
  ID相関テスト、Phase 3C-3 dry-run統合、Phase 3D前 broker / order API実装前レビューへ分割した。
- Phase 3C-1 mocked core、Phase 3C-2 ID相関テスト、Phase 3C-3 dry-run統合テストは完了。
- Phase 3D前 broker / order API実装前レビューも完了し、Phase 3D-0公式仕様・危険endpoint再レビューも完了。
  次候補はPhase 3D-1 order review model / final checklist mocked設計・実装と判定した。
...
- Phase 2F Public shadow risk/audit安定性レビュー、Phase 2Gオフライン最終デバッグ監査、Phase 3B-0、
  ...
  実注文は未実施。
- Live Verification Mode実装、order intent実装、注文API、実資金検証は未実施。
...
完了済みフェーズ。Private API read-onlyが安全に確認できた後、実注文前に独立して設計レビューした。
...
Phase 3Cでは、まだ実注文しない。Live Verification Mode実装、order intent実装、broker、注文APIも行わない。
実装設計レビューでは、次に進む場合もPhase 3C-1 mocked core実装に限定し、broker / order APIは
Phase 3D前レビューまで扱わないことを整理済みである。
...
- Phase 3B-1 mocked skeleton、Phase 3B-2 mocked endpoint tests、Phase 3B-3 preconnect review、
  Phase 3B-4 read-onlyローカル接続確認、Phase 3C Live Verification Mode設計、
  Phase 3C実装設計レビュー、Phase 3C-1 mocked core、Phase 3C-2 ID相関テスト、
  Phase 3C-3 dry-run統合テスト、Phase 3D前 broker / order API実装前レビューは完了済みである。
- 次に進む場合は、Phase 3D-1 order review model / final checklist mocked設計・実装を別タスクで扱う。
  ただし、broker、OrderRequest、注文API client、注文payload builder、Private API追加接続、APIキー確認、
  `.env`確認、実注文、実資金検証には進まない。
