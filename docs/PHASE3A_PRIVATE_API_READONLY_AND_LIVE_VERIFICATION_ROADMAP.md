...
- Phase 2F Public shadow risk/audit安定性レビュー、Phase 2Gオフライン最終デバッグ監査、Phase 3B-0、
  Phase 3B-1、Phase 3B-2、Phase 3B-3は完了した。ただし、Private API実接続、APIキー入力、注文API、
  実注文は未実施。
...
Phase 3B-4として1回だけのread-onlyローカル接続確認を別タスクで扱う。
...
- Phase 3B-1 mocked skeleton、Phase 3B-2 mocked endpoint tests、Phase 3B-3 preconnect reviewは完了済みである。
- 次に進む場合は、Phase 3B-4として1回だけのread-onlyローカル接続確認を別タスクで扱う。
