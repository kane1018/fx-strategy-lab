...
- Phase 2F Public shadow risk/audit安定性レビュー、Phase 2Gオフライン最終デバッグ監査、Phase 3B-0、
  Phase 3B-1、Phase 3B-2は完了した。ただし、Private API実接続、APIキー入力、注文API、実注文は未実施。
...
Phase 3B-3としてローカル接続前レビュー、APIキー管理手順レビュー、実接続しない運用設計確認から始める。
...
- Phase 3B-1 mocked skeletonとPhase 3B-2 mocked endpoint testsは完了済みである。
- 次に進む場合は、Phase 3B-3ローカル接続前レビューとして、実接続しないままAPIキー管理手順と運用境界を確認する。
