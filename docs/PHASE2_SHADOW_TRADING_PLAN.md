...
- **Phase 2E-3（Public ticker bid/ask provenance連携設計完了・docs-only）**: GMO Public `/v1/ticker`由来bid/askを
  `--enable-shadow-risk`経路へ安全に渡すため、`REAL_PUBLIC_BID_ASK`付与条件、kline-only synthetic reject維持、
  timestamp/freshness/skew/spread検証、MarketSnapshot案、raw response非保存、session no-network責務分離、
  fail closed、audit/summary metadata、implementation対象/非対象、test方針、acceptance criteriaを整理した。
  今回は設計のみで、実装、GMO Public run、Private API、APIキー、broker、実注文、本番公開API追加は行っていない。
