...
- **Phase 2B**: GMO Public API 仕様確認 → 実 Public API から ticker/candle 取得（local CLI で確認）。
  `MarketDataClient` を実装する adapter を追加（Public/read-only のみ。Private/APIキー禁止）。
- **Phase 2C**: shadow log 保存 / 仮想損益の蓄積 / 1〜2 週間の注文なし運用。reports 化を検討。
