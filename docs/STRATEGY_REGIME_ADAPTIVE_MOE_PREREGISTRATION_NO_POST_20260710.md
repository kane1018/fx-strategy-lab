...
`SELECTED_SPEC_PENDING` は本書だけの仕様作成 substatus であり、registry の正式状態を置き換えない。
operator による選定は完了したが、未決定項目、凍結仕様、`config_hash`、formal test の予約が未完了である。
本書は実装、データ取得、backtest、paper、live、broker/API、credential/env 読取、POSTを許可しない。
...
## 8. 未決定項目（すべてoperator判断待ち）

以下が1件でも未決定なら `frozen_spec=false`、`config_hash=NOT_ASSIGNED`、Stage 1開始不可である。
