...
### Actual transport activation boundary correction

G011までは`V4GmoHttpxPrivateTransport`のconstructorと`request()`を常時拒否していた。G012の明示授権後は、
generation/cycle-bound one-use permitとcoordinatorのDB-backed one-use action proofを両方消費した場合だけ、
fake client testでactual-shaped分岐を通せる構造へ変更した。permit単独、transport直接呼出し、別cycle、二回目action、
期限切れはrequest前に拒否する。実Keychain/networkでhard guardへ`True`を渡す操作は別actual canary Stepまで行わない。
準備用Private GETは引き続きPOST routeを持たないread-only preflightだけを使う。
...
G012 canary直前実装では、permit/runtime binding、DB-backed final transport proof、monitor-only supervisor、
enabled LaunchAgent renderer/installerを追加した。supervisorはbroker moduleへ依存せず、15秒heartbeat、
protection deadline、03:45 dispatch-required marker、04:00 flat-miss HALTだけを扱う。詳細は
`H11_V4_G012_CANARY_PREPARATION_REPORT_20260717.md`を正とする。実permit、resume、broker POSTは0のままである。
...
v4_generation_schema=H11_AUTO_GENERATION_V4_GMO_FRIDAY_LIMITED_V2
...
v4_friday_04_flat_target_monitor=implemented_monitor_plus_foreground_dispatcher_not_activated
