...
- local SQLiteへforecast/result/operator decisionを分離してappendし、検証画面でBrier / Log loss /
  方向精度、確率帯、固定候補閾値診断を表示。手動出口計画は別tableで記録し自動決済しない。
  statusは `FORWARD_SIGNAL_SCORE_ONLY_NOT_EDGE_VALIDATION`。
- 手動登録したOPEN建玉計画に対し、主画面と出口管理へposition-aware出口シグナルを追加。
  固定SL / TP / time exitを優先し、15秒価格staleは判定不可、反対正式基準2回連続は損切り候補、
  50%中立線の不利側は警戒。それ以外は継続。毎秒ローリングは出口判定へ使用しない。
- 専用entrypointはlocalhost限定で、public `main_readonly.py`、broker、Private API、credential、
  order/live経路と隔離。actual_post=false / broker_read=false / broker_write=false。
