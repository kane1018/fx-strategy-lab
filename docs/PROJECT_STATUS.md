...
- operatorは、初回監視付き10,000通貨live canaryを、actual activeOrders OCO rowsと約定後15秒以内の
  exact-size server-side protection成立の実証に兼用する方針を承認した。
- 第2 live cycleは、OCO ownership/size/15秒、duplicate/retry/repost 0、journal/risk/dead-man/notification/
  reconciliationのclear証拠が揃うまで禁止。不明または不一致はpersistent HALT。
- operator申告: Pushover 2件、email 2件、GMO v4 2件のKeychain登録済み、API権限設定済み、
  IP restrictionは無効としresidual riskを受容。値は未読・未表示・未検証。
- これはactual activation授権ではない。`actual_post=false` / `broker_read=false` / `broker_write=false` /
  `credential_read=false` / `live_ready=false` / `unattended_live_supported=false`は不変。

## 0AB. H-11 v4 activation preparation（2026-07-15・fake-only・no-POST）

- operator承認値: formal 30m `SHORT_V1`、10,000通貨、risk 5,000/10,000/50,000円、5連敗、1 entry/日、
  heartbeat 15秒 / stale 60秒、account `EXCLUSIVE_DURING_AUTO`、Pushover + email。
- exact signal config hashは既存local artifactを整合検証して固定。承認値のdigestはactivation permitではない。
- account排他、GET最大4回/秒、POST最短1.10秒、clock skew最大5秒、Pushover/email fake境界、disabled
  launchd template、有限host rehearsalを実装。
- 15秒current-Mac fake rehearsalはpass。実broker、Keychain、external notification、resident/launchd/cronは不使用。
- 検証: focused 26、h11_auto + real-POST isolation 342、禁止Keychain書込test除外backend 7911、Ruff、
  diff check、danger scan pass。
- 残件: 完全generation manifest、実credential provisioning、actual notification delivery/ack、sanitized read-only OCO
  proof、actual runtime cadence sequencing、host fault rehearsal、KILL proof、独立安全review、major-incident resume発効、
  別actual activation review。
- `actual_post=false` / `broker_read=false` / `broker_write=false` / `credential_read=false` /
  `resident_process=false` / `live_ready=false` / `unattended_live_supported=false`。
- 参照: [implementation report](H11_V4_ACTIVATION_PREPARATION_IMPLEMENTATION_REPORT_NO_POST_20260715.md)、
  [operator sheet](H11_AUTO_OPERATOR_DECISION_SHEET_NO_POST_20260715.md)。

## 0AA. H-11 Manual Signal UI（2026-07-14・local-only・no-POST）

- 自動売買から手動判断支援へ分離した専用UIを実装。画面名は `シグナル`。
- 10分を初期主表示とし、30分・24時間・毎秒ローリングを含む4枠をクリックで相互に入れ替え可能。
- 10分・30分は新規M1低容量logisticの別horizon、24時間は既存H-11 v2をcontextとして維持。
- Public WebSocket tickerのBID / ASK / spreadと実BIDローソク足を併記。正式10分・30分signalは
  M1確定後の毎分+3秒を維持し、4つ目の独立した非正式リアルタイム推定を直近60秒rollingで毎秒更新。
  1秒sampleは正式forecastと別tableへ保存し、別検証・operator承認なしには昇格しない。
- 4枠すべてに各signalのp_up履歴チャートを表示。正式3枠はPROSPECTIVE台帳、毎秒枠は画面sessionのみ。
- デスクトップでは左に価格chart、右に4signalを同時表示。chartは1分・10分・30分・1時間切替。
- 短期model未初期化・データ不足時はmock値を出さず `判定不可`。
- local SQLiteへforecast/result/signal actionを分離してappendし、検証画面でBrier / Log loss /
  方向精度、確率帯、固定候補閾値診断を表示。手動出口計画は別tableで記録し自動決済しない。
  statusは `FORWARD_SIGNAL_SCORE_ONLY_NOT_EDGE_VALIDATION`。
- 手動登録したOPEN建玉計画に対し、10分・30分の各カード内へposition-aware出口シグナルを追加。
  固定SL / TP / time exitを優先し、15秒価格staleは判定不可、反対正式基準2回連続は損切り候補、
  50%中立線の不利側は警戒。それ以外は継続。毎秒ローリングは出口判定へ使用しない。
- 手動OPEN/CLOSEは、localhost専用のGMO FX `latestExecutions / openPositions` GET-only同期で
  local出口計画へ反映する。実約定価格・数量、部分決済、全決済を同期し、曖昧・不整合時は推測せず停止する。
  専用Keychain pairが未設定なら通信不能であり、broker writeは行わない。
- 正式10分・30分カード内の取引開始に成功後はシグナル画面に留まり、主画面の出口シグナルを即時開始する。
  条件不足時はボタンを無効化し、出口管理への自動遷移や出口計画なしの取引記録を行わない。
- 正式10分・30分の各カードへ取引開始ボタンを常時表示。画面方向はBuy / Sell / Stay / Unknownへ統一し、
  開始した時間軸カードへposition-aware出口状態・約定補正・決済記録を統合。独立出口画面は削除した。
- 10分・30分は各1件までlocal手動建玉計画を独立保持し、各時間軸の出口シグナルを別々に計算・表示する。
  画面上部の建玉数は0〜2件のlocal手動登録数でありbroker建玉数ではない。24時間・毎秒は取引対象外。
- 手入力による実約定価格補正とPublic価格によるlocal決済記録UIは廃止。カードはread-only同期状態を
  表示し、broker側で手動決済されたCLOSE約定だけをlocal計画へ反映する。
- シグナル画面の `見送った / 保留 / 注文計算へ` を削除。取引開始時だけ`TRADE_STARTED`、10分・30分の
  満期時に開始記録がなければ`NO_ACTION（取引開始記録なし）`を一意に自動記録し、予測採点とは分離する。
- 独立した注文計算ページへ正式方向とfresh Public ASK/BIDを自動反映し、許容損失額から1,000通貨単位で
  数量を切り下げ逆算する。計算はlocal-onlyで注文や出口計画を開始しない。
- 専用entrypointはlocalhost限定で、public `main_readonly.py`、order/live経路と隔離。
  actual_post=false / broker_read=GET_ONLY_WHEN_DEDICATED_KEYCHAIN_PAIR_IS_CONFIGURED /
  broker_write=false。browser pageの毎分更新はあるが、server-side常駐・cronはない。
- 参照: [UI設計](H11_MANUAL_SIGNAL_UI_NO_POST.md) /
  [validation契約](H11_FORWARD_SIGNAL_VALIDATION_NO_POST.md) /
...
- no-POST自動化検証層: paper auto cycle runner / supervised auto live preview /
  ...
  評価基盤も強化済み（walk-forward＋コスト感度＋ランダムベンチマーク。M5全候補
  ...
  **記述的market-state observer（BUY/SELL出さず）** を再利用可能 platform として索引化＋**hypothesis registry
  ...
  （`operator_pre_trade_caution_briefing.py`・純粋関数・safe-aggregate・**no-private-GET/no-POST**・
...
- **Step 6G-PC-OX-R-OFFICIAL-SETTLEMENT-REJECTION-SAFE-CATEGORY-CAPTURE-INTEGRATION-NO-POST-C 完了 / safe rejection category capture integrated / no-POST** —
  ...
  と
  `backend/app/tests/test_live_verification_live_order_real_official_settlement_rejection_safe_category_capture_integration_no_post_controlled.py`
  を追加。official settlement result が `RESULT_REJECTED_SANITIZED` の場合だけ safe capture を使用し、
  final report / handoff 用に safe category / kind / source / confidence / reason availability を
  safe label / booleanのみで返す。safe detail label がない rejected-only result は
  `SAFE_REJECTION_CATEGORY_UNKNOWN` /
  `SAFE_REJECTION_KIND_BROKER_REJECTED_REASON_UNAVAILABLE` /
  `SAFE_REJECTION_SOURCE_SANITIZED_RESULT_ONLY` に落ち、
  `safe_rejection_reason_available=false`、`safe_rejection_reason_unavailable=true`、
  `safe_rejection_requires_raw_response=true` として扱う。
  safe broker code label、safe HTTP status label、operator UI safe label、official docs comparison safe result が
  ある場合だけ、category/kind/source/confidence を具体化する。actual settlement POST、entry POST、
...
- **Step 6G-PC-OX-R-SEALED-CREDENTIAL-SIGNING-PROVIDER-C sealed credential/signing/headers provider foundation完了 / no actual HTTP POST** —
  ...
  `docs/STEP6G_ONE_SHOT_POST_SEALED_CREDENTIAL_SIGNING_CONTROLLED.md` を追加した。sealed credential provider safe summary、
  sealed signing provider safe summary、sealed headers object、provider readiness summary、sealed request/body foundation
  connectionを no-POST / no-credential-value / no-signature-value / no-headers-value / no-ledger / no-receipt のまま実装した。
  新規moduleは `live_order_once`、broker/private API、HTTP client、env reader、credential reader、real signing helper、
  real header builder、ledger writer、receipt handoffをimport/callしない。sentinel testsでrepr/asdict/rendererがcredential値、
  credential length/hash/fingerprint/metadata、signature値、signature length/hash/fingerprint、headers値、headers
  metadata/count、raw body、raw response、broker/API response、IDを出さないことを確認する。
  `approved_primitive_actual_source_available` はfalseのままで、actual source callableやledger-free source factoryは未実装。
  次の推奨Stepは **Step 6G-PC-OX-R-LEDGER-FREE-POST-ONLY-SOURCE-FACTORY-C**。次Stepでもactual HTTP POST、
...
- **Step 6G-PC-OX-R-ONE-SHOT-POST-APPROVED-PRIMITIVE-SOURCE-SUPPLY-C approved primitive source supply boundary implementation完了 / no actual HTTP POST** —
  ...
  次の推奨Stepは **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-GATE-RETRY-5**。次Stepでも最初からPOSTせず、safe preview提示後に
  このCodexセッション内の新しいPOST-specific confirmationを取得してから、条件が揃う場合だけ最大1回のHTTP POSTを検討する。
  RETRY-4のPOST-specific confirmationは再利用不可で、ledger/receipt/retry/repostは引き続き分離必須。
...
- **Step 6G-PC-OX-R-ONE-SHOT-POST-REAL-TRANSPORT-BINDING-C controlled real transport binding implementation完了 / no actual HTTP POST** —
  ...
  sanitized transport outcome mappingを提供する。新規moduleは `live_order_once`、broker/private API、HTTP client、
  env reader、ledger writer、receipt handoffをimport/callしない。testsではfake/monkeypatch primitiveのみを使い、
  controlled executor connection、one POST max compatibility、no retry、timeout fail-closed、ledger/receipt分離、
  raw/ID/value非露出を確認する。このStepではactual HTTP POST、order endpoint、`live_order_once`、
  POST-specific confirmation取得、ledger update、attempt counter persistence、actual receipt handoff、retry/repost、
  fresh preflight再実行、final confirmation再取得には進んでいない。次の推奨Stepは
  **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-GATE-RETRY-2**。次Stepでも最初からPOSTせず、safe preview提示後に
  このCodexセッション内の新しいPOST-specific confirmationを取得してから、条件が揃う場合だけ最大1回のHTTP POSTを検討する。
...
- **Step 6G-PC-OX-R-ONE-SHOT-POST-EXECUTION-RUNTIME-C safe one-shot POST execution route implementation完了 / no actual HTTP POST** —
  ...
  `docs/STEP6G_ONE_SHOT_POST_EXECUTION_CONTROLLED.md` を追加した。routeはsanitized executable order preview、
  POST-specific confirmation safe validator、transport注入型one-shot controlled executorを提供するが、route ready状態は
  POST許可ではなく、actual transport bindingと新しいPOST-specific confirmationが別途必要。
...
- **Step 6G-PC-OX-R-FRESH-PREFLIGHT-EXECUTION-RUNTIME-E actual safe fresh preflight execution mode implementation完了 / execute mode implemented / no actual fresh preflight run / no HTTP POST / no final confirmation** —
  ...
  `backend/app/live_verification/run_fresh_preflight_execution_controlled.py` を更新した。default / adapter-summary CLIは
...
- **Step 6G-PC-C real credential presence checker contract完了 / no env / no actual credential / no API / no POST** —
  ...
  新しいfinal confirmationとfresh preflightが必要。詳細は
  [STEP6G_REAL_CREDENTIAL_PRESENCE_CHECKER_CONTRACT.md](STEP6G_REAL_CREDENTIAL_PRESENCE_CHECKER_CONTRACT.md)。
- **Step 6G-PC-A credential presence adapter skeleton完了 / no env / no actual credential / no API / no POST** —
...
operator current-turn 5項目完全一致を受け、read-only runtime safe confirmation を
1回のみ実行（既存監査済み `check_private_readonly_connection.py` 経由・sanitized のみ）:
...
## H-11 v3 Broker Response and v4 Constraint Redesign（2026-07-13・no-POST）

- operator共有のGMOサポート回答を、raw文面を保存せず
  [安全要約](H11_V3_GMO_SUPPORT_RESPONSE_SAFE_SUMMARY_NO_POST_20260713.md)に記録。
- IFDOCO一次注文は固定30取引日でrequestごとの短縮expiryなし、部分約定時の第二OCO sizeは
  実約定量へ自動調整されないことが判明。
- このためv3は`BLOCKED_V3_BROKER_CONSTRAINTS`。短期signal windowを超えるpending entryと、
  部分約定後の保護size不一致を現行凍結specでは安全に解消できない。
- v3 configは変更せず、実行profile未選定の
  [v4 docs-only再設計草案](H11_V4_BROKER_CONSTRAINT_REDESIGN_DRAFT_NO_POST_20260713.md)へ移行。
- actual POST / broker read/write / credential read / resident process / cronはすべてfalse。

## H-11 Manual UI Every-Second Rolling Validation（2026-07-15・local-only / no-POST）

- 10分・30分の毎秒ローリング推定を、正式forecastとは別のSQLite台帳へ時点固定で保存。
- origin BIDと対象時刻のBIDを比較し、対象時刻+15秒以内だけ解決。超過は
  `TARGET_PRICE_MISSING`として固定し、後知恵の価格補完を禁止。
- 検証UIへraw / horizon間隔の非重複指標、Brier / Log loss / calibration / 閾値診断、
  解決coverage、欠測数、推定modeを追加。
- 非正式、promotion不許可、閾値自動変更なし、出口シグナル非接続を維持。
- actual POST / broker read/write / Private API / credential・env readはすべてfalse。

## H-11 Manual Settlement Read-Only Sync（2026-07-15）

- local manual UI専用にGMO FX `latestExecutions / openPositions` GET-only同期を実装。
- 専用Keychain pair未設定時は通信不能。注文・取消・変更・決済・ws-auth endpointは到達不能。
- raw IDはtransport内でkeyed opaque refへ変換し、sanitized SQLiteへ約定種別・数量・価格・時刻だけ保存。
- OPEN一意照合、部分CLOSE累積、全CLOSE反映、重複poll冪等化、曖昧・不整合時停止をfake clientで検証。
- 手入力約定補正・local決済記録UIを削除。カードへ同期状態、上部へlocal/broker件数を追加。
- `Stay=青`、`Buy/Sell=黄`。`main_readonly.py`は未変更。actual POST / broker writeはfalse。

## H-11 Manual UI One-Click Exit Start（2026-07-15・local-only / no-POST）

- 正式10分・30分の買い/売りと15秒以内のPublic価格に限定して、取引記録と出口管理を1クリック化。
- 買いASK / 売りBIDを参考entryとし、固定SL 15pips / TP 22.5pips / time exitを即時設定。
- 正式forecast方向一致、対象時刻未到達、fresh sample / market time、同時OPEN 1件を強制。
- 二重クリックをUIで遮断。不成立時は従来の実約定価格入力フォームへfail closed。
- broker fill確認、自動決済、actual POST、broker read/write、credential・env readは追加していない。

## H-11 v4 actual activation preparation corrective cycle（2026-07-16）

OperatorはKeychain値非表示確認、通知実配送、3経路Private GET、非破壊host/KILL試験、独立reviewまでを
授権した。AGENTS.mdへ準備限定例外を追加し、通知actual sender、POST不能のGET-only preflight、
presence-only Keychain check、persistent KILL rehearsal、OCO確認時15秒再検査をfake-firstで実装した。

初回generationではpresence 6/6後、通知送信前のKeychain internal readが5秒の対話timeoutで停止した。
login keychainはdefaultかつ`no-timeout`であり、Pushover/email、Private GET、broker POSTは0。
既存markerは削除せず保持する。

修正実装は通知前にoperation合計最大120秒のKeychain access rehearsalを追加し、state rootをreviewed-files digestごとの
新generationへ分離した。同一digestの失敗operationは再試行不能で、旧generationのreset・削除・上書きを
