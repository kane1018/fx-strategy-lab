# H-11 v4 G076 Runtime-only Corrective Contract
...
them authorizes G076. G076 uses a distinct digest and state root, and no G076
...
request-specific. G076 does not weaken TLS, disable verification, add fallback
transport, or retry the G074 request.

Before the one-use transaction marker, G076 may perform exactly one
credential-free GET to the fixed same-origin public status URL. It records only
one of the fixed diagnostic labels `DNS_FAILURE`, `TCP_CONNECT_FAILURE`,
`TLS_HANDSHAKE_FAILURE`, `TLS_CERTIFICATE_FAILURE`, `CONNECT_TIMEOUT`,
`READ_TIMEOUT`, `PRIVATE_REQUEST_BUILD_FAILURE`, `AUTH_HEADER_BUILD_FAILURE`,
`HTTP_STATUS_FAILURE`, `RESPONSE_SCHEMA_FAILURE`, or
`UNKNOWN_NETWORK_FAILURE`. A failed preflight does not start the transaction.
...
The initial G076 activation is a one-shot transaction. It creates the durable
...
- exact G076 generation label and generation digest;
...
No action is authorized by a boolean assembled from readiness flags. G076
...
G076 is not reviewable until deterministic fake tests prove all of the
...
1. commit and push the reviewed G076 implementation;
2. promote the exact G076 artifact and commit/push promotion;
3. execute G076 operation 60 once;
4. verify resident readiness;
5. execute the G076 initial activation transaction once;
...
Operation 60 or initial activation `UNKNOWN` is terminal for G076. No retry,
marker reset, or automatic successor generation is permitted.

## Definition of done

G076 is complete only when the fake acceptance matrix, related tests, Ruff,
