...
2. manifest / warnings / summary / metrics writer の共通化（`fx_eval_common` 拡張）
   ...
   `breakout_15window.py`（2本目）、`bollinger_15window.py`（3本目・TP拡張CSV＋market-state別CSVを
   `write_metrics_csv(..., stat_fields=BK_STAT_FIELDS)` で対応、final_decision は `write_markdown`）。
   いずれも出力内容は不変・mechanism のみ共通化。残り（market_structure / rsi_m15 / rsi_m15_scaled /
   regime診断）は次段階。
