...
## 2026-07-22 GUI-domain pre-marker guard review

- Independent architecture/safety/operations review: `APPROVE_WITH_CONDITIONS`, VETOなし。
- preflightはledger.begin前、read-only `launchctl print gui/<uid>`だけに限定する。
- anomaly、timeout、marker欠落は単一固定errorへ写像し、raw launchctl outputを表示・保存しない。
- retry-safe refusalとpost-marker no-retry failureを別statusに固定する。
- healthy shapeはmacOS 15.7.7 / Darwin 24G720の実機steady-state出力で確認済み。
- fake runnerでhealthy、nonzero、empty、各marker欠落、timeoutを固定し、script-level testでgate/ledger/install、
  JSON marker、plistがpreflight refusal時に未到達であることを固定する。
- login transitionがpreflight直後に始まるTOCTOUは残るため、50秒heartbeat gateを最終fail-closed検出器として維持する。
