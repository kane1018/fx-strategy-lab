...
## 9. Friday-limited v2 schedule

```text
Monday-Thursday entry=JST 00:00-04:59 and 09:00-23:59
Friday entry=JST 09:00-inclusive to 21:00-exclusive
Saturday/Sunday entry=blocked
normal time exit=entry attempt + 82,800 seconds
Friday exit-sequence start=min(normal time exit, Saturday 03:45 JST)
Friday flat target=Saturday 04:00 JST
```

金曜のweekend flatは土曜03:45から、exact OCO取消、fresh reconciliation、position-specific closeの
各one-attemptを固定順序で開始し、04:00をflat目標とする。03:45前のweekend exit、同一actionのretry/repost、
status `OPEN`が不明または2秒超の証拠によるwriteは拒否する。04:00時点でflatを確認できない場合は、
OCOを維持できる状態では維持し、自動再試行せずpersistent HALTする。broker障害・市場CLOSED/UNKNOWN時の
flat完了を保証するものではない。03:45自動dispatcherと04:00 target monitorはactivation前の凍結必須要件で、
本refreeze時点では未発効である。このschedule変更はactual activationやPOSTを許可しない。
