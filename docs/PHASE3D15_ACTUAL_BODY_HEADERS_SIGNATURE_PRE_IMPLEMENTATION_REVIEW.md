...
## 16. Step 1統合 / Phase 3D-16D actual headers / signature + mock transport 結果メモ

Phase 3D-16Dでは、Phase 3D-16A/B/Cの成果を統合し、`ActualOrderRequestBody` の後段に
`ActualHeadersSignatureBundle` と `MockSignedOrderTransportResult` を追加した。

完了した範囲:

- actual bodyの署名用stable JSON serialization。
- actual headers / signatureの最小bundle。
- HMAC-SHA256 hex署名。
- private redacted objectによるsensitive headers / signature保持。
- mock signed transportによるno-network境界確認。
