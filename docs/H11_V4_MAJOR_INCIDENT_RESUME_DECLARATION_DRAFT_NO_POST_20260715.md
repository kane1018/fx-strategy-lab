...
```text
applies_only_to=H11_V4_GMO_MARKET_THEN_EXACT_OCO_FRIDAY_LIMITED_NO_POST_V2
applies_to_v3=false
...
この文の一致だけでactivation permitを生成してはならない。G012 generation/cycleへ結合したfresh gate、
独立レビュー、ランダムnonceを含むcurrent-turn exact confirmationを別に必要とする。
...
g012_canary_preparation_authorized=true
