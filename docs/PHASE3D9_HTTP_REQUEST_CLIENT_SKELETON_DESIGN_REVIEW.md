...
## 13. Phase 3D-11 preflightレビュー結果メモ

Phase 3D-11では、実注文へ進む前のpreflight条件をdocs-onlyで整理した。

整理したもの:

- 実注文直前preflightの定義。
- `OrderIntent` から `DisabledHttpRequestClientSkeletonPlan` までのno-network chain。
- 直前read-only precheck条件。
- 技術チェックリスト。
- 注文条件チェックリスト。
- APIキー / secret / `.env` を扱う前の条件。
- HTTP request / 署名実装へ進む前の条件。
- ユーザー明示承認の形式。
- 即停止条件。
- 実注文後の停止・確認条件。
- Phase 3D-12以降の分割案。
...
次候補はPhase 3D-12 APIキー確認専用レビューである。ただし、Phase 3D-12でもAPIキー値やsecret値は表示せず、
`set` / `missing` のみを扱うレビューとして分離する。
