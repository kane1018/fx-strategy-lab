...
## 15. Phase 3D-15レビュー結果メモ

Phase 3D-15では、Phase 3D-14Bまでで強化した `SignatureHeadersBodyPlan` を前提に、
actual request body、actual headers、actual signatureの最小実装へ進む前の責務分離と安全境界を
docs-onlyで整理した。

作成したdocs:

- `docs/PHASE3D15_ACTUAL_BODY_HEADERS_SIGNATURE_PRE_IMPLEMENTATION_REVIEW.md`

整理した内容:

- actual request body生成の責務。
- actual headers生成の責務。
- actual signature生成の責務。
- body builder / signer / headers builder / HTTP clientの責務分離。
- APIキー値、secret値、headers値、signature値を表示・保存・commitしない運用。
- raw request / raw response / headers / signatureを保存しない運用。
- HTTP POST、broker、`OrderRequest`、real order API client、実注文へ進まない境界。
- Phase 3D-16以降の分割案。
...
- ただしPhase 3D-16でも、HTTP POST、HTTP request送信、Private API追加接続、broker、`OrderRequest`、
  real order API client、実注文、実資金検証には進まない。
