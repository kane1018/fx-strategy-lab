...
実資金取引はデフォルトで無効です。`app/brokers/` と `app/services/risk_service.py` を通る
通常の注文経路（`OrderRequest` / `RiskConfig`）には実資金ブローカー実装を含めず、
ライブ注文要求は安全条件に関係なく最終的に拒否されます。

**重要な例外（2026-07-06 監査で確認）**: `backend/app/live_verification/live_order_once.py`
とその周辺モジュール（`live_order_real_official_settlement_actual_transport_no_post_controlled.py`、
`live_order_real_one_shot_post_real_delegate_controlled.py`）は、上記の通常経路とは別系統で、
GMO FX本番Private APIへの実HMAC署名付きHTTP POSTを実行できる**実装済みの実コード**です。
これらは `risk_service.evaluate_order_risk()` の無条件拒否ロジックを経由しません。
現状は明示的な transport・実credential・`allow_live_http_post=True` 相当のフラグをすべて
呼び出し側が渡さない限り発火しない設計で、Step 6G の "controlled/safe" 系モジュールの
既定エントリポイントからは到達できないことを回帰テスト
（`backend/app/tests/test_live_verification_real_post_capability_isolation.py`）で固定していますが、
「実資金ブローカー実装を含めない」という表現はこのモジュール群には当てはまりません。
詳細は [docs/CODEX_HANDOFF.md](docs/CODEX_HANDOFF.md) のインシデント記録を参照してください。
...
- GMO 外国為替FX には**デモ環境がなく本番のみ**のため、`GmoFxBroker` 経由の実注文は別フェーズで
  明示許可があるまで無効です。ただし `GmoFxBroker` とは別に、
  `backend/app/live_verification/live_order_once.py` に実POST可能な低レベル実装が存在します
  （冒頭「セットアップ」節末尾の注記、[docs/CODEX_HANDOFF.md](docs/CODEX_HANDOFF.md) 参照）。
- Private read-only（account/assets・openPositions・activeOrders）の実接続は
  `backend/scripts/check_private_readonly_connection.py` + `app/private_api/` に実装済みです。
