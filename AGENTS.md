...
- previewはlocal M1とfresh M1をメモリ内だけで結合し、凍結`SHORT_V1`／30分閾値で
  `candidate_actionable`だけを判定してよい。direction、probability、price、raw candleは表示・保存せず、
  candle CSV、actual preparation state、actual runtime state、formal-candles markerを変更しない。
...
- actual sessionのfresh正式30分signalと凍結risk幅を作るため、Public `klines`の当日M1とH1を各1回だけ
  GETしてよい。credential／Private API／broker writeを使わず、same-request retryを行わない。
