...
## H-11 v4 G077 unknown-resolution read-back corrective generation限定例外

operatorがG077 corrective generation（`H11_AUTO_30M_20260805_G077`）の設計承認
（D1=(a)次cycle新規観測 / D2=(b)全write action共通 / v1.1レビュー反映H1-H2-M1-M2-M3-L1-L2）
と一括実装を明示承認した場合に限り、以下のno-POST・fake-only実装を行ってよい。
G077は「UNKNOWN=generation terminal」が産む不要な新generationパイプラインを、同generation内
read-back解決でレアパスへ縮小する。実Keychain、Private API、broker GET/POST、注文、取消、
変更、決済、通知、ARM変更、LaunchAgent操作は一切行わない。

### G077で限定的に許可すること

- read-back解決step（`backend/app/services/h11_v4_g077_runtime.py`）を実装する。writeは
  各action1回のまま、UNKNOWN直後に`latestExecutions`／`openPositions`／`activeOrders`を
  各最大1回readし、action kind別にCONFIRMED_EXECUTED／CONFIRMED_NOT_EXECUTED／
  CONFIRMED_PARTIAL_FILL／UNRESOLVEDへfail-closed分類する。
- 解決marker・evidenceはaction_scope_digest単位（`g077-resolution.{scope}.started/evidence.json`）、
  同一generationの解決予算は最大3（各scope1回）、GET間隔は0.25秒以上、開始窓15秒・完了予算
  60秒・injected monotonic clock・flock工程ロックを実装する。UNRESOLVED／PARTIAL／TERMINAL
  policyのみpersistent HALT。CONFIRMED_NOT_EXECUTEDは次scheduled cycle（新M1 slot）の
  新規観測としてのみ再評価できる。
- G076 promotion commit 1bd9ef7がcanonical templateへ追加したpredecessor provenance 3キー
  （`predecessor_generation_digest`／`predecessor_initial_activation_unknown`／
  `predecessor_terminal_evidence_reused`）をdataclassが受け入れるよう
  `v4_gmo_generation.py`を修正する（pop-if-None・weak validation・digest不変）。
  これに伴う共有reviewed digest変化に対して、G076 canonical template・G076 candidate
  template・commissioning evidence・attestationのdigest参照をlockstep再束縛する。
  対象は未activationのtemplateファイルのみであり、runtime marker／HALT／state rootは存在せず
  変更しない。G076 generation digestは修正後に新値（`sha256:…`）へ更新される。
- G077専用reviewed-files digest module（`backend/h11_v4_g077_reviewed_digest.py`）と
  generation template（`docs/templates/h11_v4_g077_frozen_generation.json`）を新設し、
  G077 reviewed digest / generation digestへ再凍結する。共有`h11_v4_reviewed_digest.py`の
  既存束縛は変更しない（共有digest値自体は上記dataclass修正により変化する）。
- fake/synthetic入力だけのacceptance tests（scope単位marker・予算・pacing・タイムライン・
  lock・action kind別分類・部分約定・source scan含む）を追加する。実装moduleからの
  httpx／Keychain／smtplib／subprocess／requests／Pushover到達をsource scanで固定禁止する。
- 設計doc（`docs/H11_V4_G077_UNKNOWN_RESOLUTION_DESIGN.md`）・AGENTS.md本節・
  A-2/A-3のCI復旧（G076 regressionのCWD非依存化・stale manual test期待値更新）を
  必要最小限で行う。CI復旧には以下2件の既存失敗の修正も含める：
  (1) `h11_v4_gmo_actual_transport.py`の`request()`が未初期化オブジェクトで
  `_recovered_exit_only`へ先に到達しAttributeErrorになる防御順序バグを
  `getattr(self, "_recovered_exit_only", False)`でfail-closedに強化する（弱化なし）。
  (2) terminal世代の`h11_v4_g074_live_runtime.py`／`h11_v4_g075_live_runtime.py`の
  `_run_entry_actual`が実credential/実transportを構築して`run_unattended_live_entry_cycle_once`を
  直接呼ぶsingle-caller契約違反を、固定label（`G074_GENERATION_TERMINAL_NO_ENTRY`／
  `G075_GENERATION_TERMINAL_NO_ENTRY`）でraiseするterminal無効化へ置換し、当該import
  （orchestration・notification actual transport/preparation）を除去する。
  これらはG074/G075のruntime marker・HALT・state root・evidenceの変更ではなく、
  未実行legacyコード経路の構造的除去である。
- evidenceへ`actual_post_authorized=false`／`broker_post_authorized=false`／
  `entry_authorized=false`を固定し、`__bool__`は常にfalse、解決結果からaggregate allow・
  permit・hard-guard解除値を生成しない。
...
- G073/G074/G075/G076のruntime marker・HALT・state root・evidenceの変更・削除・reset、
  またはauthorization再利用。G078を自動で作成しない。
- 本例外下の実装・test完了をlive-ready、performance proof、activation承認として扱うこと。
