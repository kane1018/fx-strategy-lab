from __future__ import annotations

import ast
from dataclasses import asdict, replace
from pathlib import Path

import pytest

from app.live_verification.live_order_real_credential_boundary import (
    LiveOrderRealCredentialBoundaryInput,
    LiveOrderRealCredentialBoundaryStatus,
    build_live_order_real_credential_boundary,
    render_live_order_real_credential_boundary_markdown,
)

Status = LiveOrderRealCredentialBoundaryStatus


def _input(**overrides: object) -> LiveOrderRealCredentialBoundaryInput:
    base = LiveOrderRealCredentialBoundaryInput()
    return replace(base, **overrides)


def _build(**overrides: object):
    return build_live_order_real_credential_boundary(
        input_snapshot=_input(**overrides),
    )


def test_valid_boundary_only_input_ready_no_credential_no_env() -> None:
    result = _build()

    assert result.status is Status.CREDENTIAL_BOUNDARY_READY_NO_CREDENTIAL_NO_ENV
    assert result.credential_boundary_ready is True
    assert result.boundary_mode == "BOUNDARY_ONLY"
    assert result.real_credentials_requested is False
    assert result.credential_values_provided is False
    assert result.credential_values_loaded is False
    assert result.credential_presence_checked_against_environment is False
    assert result.env_access_requested is False
    assert result.dotenv_access_requested is False
    assert result.printenv_requested is False
    assert result.credential_metadata_exposed is False
    assert result.credential_values_displayed is False
    assert result.credential_values_saved is False
    assert result.credentials_safe_to_render is True
    assert result.credentials_safe_to_serialize is True
    assert result.can_generate_real_signature is False
    assert result.can_generate_real_headers is False
    assert result.can_execute_http_post is False
    assert result.http_post_executed is False
    assert result.order_endpoint_called is False
    assert result.live_order_once_called is False
    assert result.post_allowed_this_step is False
    assert result.post_executed is False
    assert result.retry_allowed is False
    assert result.loop_allowed is False


@pytest.mark.parametrize(
    "overrides",
    [
        {"boundary_mode": "REAL_CREDENTIAL_BOUNDARY"},
        {"signing_contract_ready": False},
        {"dummy_signing_ready": False},
        {"http_transport_interface_ready": False},
    ],
)
def test_input_blockers(overrides: dict[str, object]) -> None:
    result = _build(**overrides)

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_INPUT
    assert result.credential_boundary_ready is False


def test_real_credential_requested_blocks() -> None:
    result = _build(real_credentials_requested=True)

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_REAL_CREDENTIAL_REQUESTED
    assert "real_credentials_requested" in result.blocked_reasons
    assert result.real_credentials_requested is False


def test_credential_value_provided_blocks() -> None:
    result = _build(credential_values_provided=True)

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_VALUE_PROVIDED
    assert "credential_values_provided" in result.blocked_reasons
    assert result.credential_values_provided is False


def test_credential_value_loaded_blocks() -> None:
    result = _build(credential_values_loaded=True)

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_VALUE_LOADED
    assert "credential_values_loaded" in result.blocked_reasons
    assert result.credential_values_loaded is False


@pytest.mark.parametrize(
    "field_name",
    [
        "credential_presence_checked_against_environment",
        "env_access_requested",
        "dotenv_access_requested",
        "printenv_requested",
    ],
)
def test_env_or_presence_access_blocks(field_name: str) -> None:
    result = _build(**{field_name: True})

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_ENV_ACCESS


@pytest.mark.parametrize(
    "field_name",
    [
        "credential_length_available",
        "credential_hash_available",
        "credential_fingerprint_available",
        "credential_preview_available",
        "credential_prefix_available",
        "credential_suffix_available",
    ],
)
def test_credential_metadata_exposure_blocks(field_name: str) -> None:
    result = _build(**{field_name: True})

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_METADATA_EXPOSURE
    assert result.credential_metadata_exposed is False


@pytest.mark.parametrize(
    "overrides",
    [
        {"credential_values_displayed": True},
        {"credential_values_saved": True},
        {"credentials_safe_to_render": False},
        {"credentials_safe_to_serialize": False},
    ],
)
def test_display_save_or_unsafe_serialization_blocks(
    overrides: dict[str, object],
) -> None:
    result = _build(**overrides)

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_DISPLAY_OR_SAVE


@pytest.mark.parametrize(
    "field_name",
    [
        "can_generate_real_signature",
        "can_generate_real_headers",
        "can_execute_http_post",
        "http_post_executed",
        "order_endpoint_called",
        "live_order_once_called",
        "post_allowed_this_step",
        "post_executed",
    ],
)
def test_real_signing_headers_or_post_blocks(field_name: str) -> None:
    result = _build(**{field_name: True})

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_REAL_SIGNING_OR_POST


@pytest.mark.parametrize("field_name", ["retry_allowed", "loop_allowed"])
def test_retry_or_loop_is_unsupported(field_name: str) -> None:
    result = _build(**{field_name: True})

    assert result.status is Status.BLOCKED_CREDENTIAL_BOUNDARY_UNSUPPORTED


def test_renderer_includes_required_warnings_without_credential_metadata() -> None:
    result = _build()
    rendered = render_live_order_real_credential_boundary_markdown(result)

    assert "This credential boundary is boundary-only." in rendered
    assert "does not use real credentials" in rendered
    assert "does not access env or .env" in rendered
    assert "does not expose credential metadata" in rendered
    assert "does not generate real signatures" in rendered
    assert "does not execute API calls" in rendered
    assert "does not execute HTTP POST" in rendered
    assert "Future real credential injection must be a separate Step." in rendered
    assert "REAL_CREDENTIAL_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_LENGTH_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_HASH_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_FINGERPRINT_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_PREVIEW_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_PREFIX_VALUE_SENTINEL" not in rendered
    assert "CREDENTIAL_SUFFIX_VALUE_SENTINEL" not in rendered
    assert "RAW_REQUEST_SENTINEL" not in rendered
    assert "RAW_RESPONSE_SENTINEL" not in rendered


def test_asdict_does_not_contain_credential_values_or_metadata_fields() -> None:
    result = _build()
    payload = repr(asdict(result))

    assert "REAL_CREDENTIAL_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_LENGTH_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_HASH_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_FINGERPRINT_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_PREVIEW_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_PREFIX_VALUE_SENTINEL" not in payload
    assert "CREDENTIAL_SUFFIX_VALUE_SENTINEL" not in payload
    assert "credential_length_available" not in payload
    assert "credential_hash_available" not in payload
    assert "credential_fingerprint_available" not in payload
    assert "credential_preview_available" not in payload
    assert "credential_prefix_available" not in payload
    assert "credential_suffix_available" not in payload
    assert "RAW_REQUEST_SENTINEL" not in payload
    assert "RAW_RESPONSE_SENTINEL" not in payload


def test_new_module_does_not_import_env_http_private_broker_or_live_order_once() -> None:
    path = (
        Path(__file__).resolve().parents[1]
        / "live_verification"
        / "live_order_real_credential_boundary.py"
    )
    tree = ast.parse(path.read_text(encoding="utf-8"))
    blocked_modules = {
        "requests",
        "httpx",
        "aiohttp",
        "urllib",
        "urllib3",
        "http.client",
        "socket",
        "subprocess",
        "dotenv",
        "app.brokers",
        "app.private_api",
        "app.live_verification.live_order_once",
    }
    blocked_names = {
        "getenv",
        "ENABLE_LIVE_TRADING",
        "GMO_FX_API_KEY",
        "GMO_FX_API_SECRET",
        "Authorization",
        "OrderRequest",
        "speedOrder",
        "changeOrder",
        "cancelOrders",
        "closeOrder",
        "live_order_once",
        "pbcopy",
    }
    blocked_attrs = {"environ", "getenv"}

    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            assert all(
                not _is_blocked_module(alias.name, blocked_modules)
                for alias in node.names
            )
        if isinstance(node, ast.ImportFrom):
            module = node.module or ""
            assert not _is_blocked_module(module, blocked_modules)
        if isinstance(node, ast.Name):
            assert node.id not in blocked_names
        if isinstance(node, ast.Attribute):
            assert node.attr not in blocked_attrs


def _is_blocked_module(module: str, blocked_modules: set[str]) -> bool:
    return any(
        module == blocked or module.startswith(f"{blocked}.")
        for blocked in blocked_modules
    )
