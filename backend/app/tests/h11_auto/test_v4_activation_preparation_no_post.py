...
def test_private_api_cadence_is_conservative_and_never_queues_or_retries() -> None:
    ...
    assert v4_reconciliation_get_offsets_seconds() == (0.0, 0.25, 0.50)
    assert gate.admit(method=V4CadenceMethod.PRIVATE_GET, now_monotonic=10.0) is True
    assert gate.admit(method=V4CadenceMethod.PRIVATE_GET, now_monotonic=10.24) is False
    assert gate.admit(method=V4CadenceMethod.PRIVATE_GET, now_monotonic=10.25) is True
    ...
    with pytest.raises(V4ActivationPreparationError, match="GET cadence"):
        V4PrivateApiCadenceGate(get_minimum_interval_seconds=0.24)
