"""Fake-only acceptance tests for the G078 unknown-resolution read-back (v1.1)."""
...
from h11_v4_g078_reviewed_digest import (
    G078_REVIEWED_FILES,
    ...
    *,
    ...
    configured = {
        G078ReadBackSource.LATEST_EXECUTIONS: (
            ...
            else _read(source=G078ReadBackSource.LATEST_EXECUTIONS, known=True)
        ),
...
class _Clock:
    ...
    def monotonic(self) -> float:
        ...
        if self._jump_after is not None and self._calls > self._jump_after:
            return self.now + G078_RESOLUTION_COMPLETION_BUDGET_SECONDS + 10.0
        return self.now
...
def _run(
    ...
    *,
    ...
    return run_g078_unknown_resolution_once(
        state_root=tmp_path,
...
def _evidence(tmp_path: Path, *, scope: str = SCOPE) -> dict[str, object]:
    return load_g078_resolution_evidence(state_root=tmp_path, action_scope_digest=scope)
...
def test_confirmed_executed(tmp_path):
    ...
    assert _run(tmp_path, client) == G078ResolutionState.CONFIRMED_EXECUTED.value
    assert not (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    evidence = _evidence(tmp_path)
    assert evidence["status"] == G078ResolutionState.CONFIRMED_EXECUTED.value
    ...
    assert (
        tmp_path / f"g078-resolution.{SCOPE[7:]}{G078_RESOLUTION_STARTED_SUFFIX}"
    ).is_file()


def test_confirmed_not_executed_next_cycle(tmp_path):
    client = _client()  # default: flat + zero orders + no match
    assert _run(tmp_path, client) == G078ResolutionState.CONFIRMED_NOT_EXECUTED.value
    assert not (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    evidence = _evidence(tmp_path)
...
def test_partial_fill_is_terminal_halt(tmp_path):
    ...
    assert _run(tmp_path, client) == G078ResolutionState.CONFIRMED_PARTIAL_FILL.value
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_PARTIAL_FILL_TERMINAL"
    evidence = _evidence(tmp_path)
...
def test_first_read_failure_is_unresolved_halt_and_stops_reading(tmp_path):
    calls: dict[str, int] = {}
    client = _client(
        executions=_read(source=G078ReadBackSource.LATEST_EXECUTIONS, known=False),
        calls=calls,
    )
    assert _run(tmp_path, client) == G078ResolutionState.UNRESOLVED.value
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_READ_BACK_UNKNOWN"
    assert set(calls) == {G078ReadBackSource.LATEST_EXECUTIONS.value}
    evidence = _evidence(tmp_path)
...
def test_ambiguous_observation_is_unresolved_halt(tmp_path):
    ...
    assert halt["reason"] == "G078_RESOLUTION_AMBIGUOUS"


def test_scope_marker_blocks_retry(tmp_path):
    client = _executed_entry_client()
    assert _run(tmp_path, client) == G078ResolutionState.CONFIRMED_EXECUTED.value
    with pytest.raises(G078Error, match="G078_RESOLUTION_ALREADY_STARTED_NO_RETRY"):
...
def test_different_scope_allowed_within_budget(tmp_path):
    ...
    # Budget is derived from the per-scope evidence files: no ledger exists,
    # and the status projection reports the evidence-derived count (C2).
    assert not list(tmp_path.glob("g078-resolution.budget*.json"))
    status = g078_resolution_status(state_root=tmp_path)
    assert status["resolutions_used"] == 2
    assert status["resolutions_limit"] == G078_MAX_RESOLUTIONS_PER_GENERATION
...
def test_budget_exceeded_after_three_scopes(tmp_path):
    ...
    with pytest.raises(G078Error, match="G078_RESOLUTION_BUDGET_EXCEEDED"):
        _run(tmp_path, _executed_entry_client(), scope=SCOPE_D)
    # C1: a refused resolution leaves the underlying UNKNOWN unresolvable, so
    # the refusal latches a persistent halt.
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_RESOLUTION_BUDGET_EXHAUSTED_TERMINAL"


def test_start_window_exceeded_latches_halt(tmp_path):
    client = _executed_entry_client()
    observed = NOW - timedelta(
        seconds=int(G078_RESOLUTION_START_WINDOW_SECONDS) + 1
    )
    with pytest.raises(G078Error, match="G078_RESOLUTION_START_WINDOW_EXCEEDED"):
        _run(tmp_path, client, observed=observed)
    # The resolution never started (no marker, no evidence) but the underlying
    # UNKNOWN is unresolvable inside the safety window: terminal (C1).
    assert not list(tmp_path.glob(f"g078-resolution.*{G078_RESOLUTION_STARTED_SUFFIX}"))
    assert not list(tmp_path.glob(f"g078-resolution.*{G078_RESOLUTION_EVIDENCE_SUFFIX}"))
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_RESOLUTION_START_WINDOW_EXCEEDED"


def test_monotonic_start_window_enforced(tmp_path):
    client = _executed_entry_client()
    clock = _Clock()
    # Wall clock is fresh, but the caller's monotonic observation is 100s old:
    # the monotonic bound must refuse and latch the halt (C3/LOW-3).
    observed_monotonic = clock.monotonic() - 100.0
    with pytest.raises(G078Error, match="G078_RESOLUTION_START_WINDOW_EXCEEDED"):
        run_g078_unknown_resolution_once(
            state_root=tmp_path,
            generation_digest=GEN,
...
def test_completion_timeout_is_unresolved_halt(tmp_path):
    ...
    client = _executed_entry_client()
    assert _run(tmp_path, client, clock=clock) == G078ResolutionState.UNRESOLVED.value
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_RESOLUTION_TIMEOUT"
    evidence = _evidence(tmp_path)
...
def test_read_pacing_enforces_min_interval(tmp_path):
    ...
    assert clock.sleeps and all(
        value >= G078_MIN_READ_INTERVAL_SECONDS - 1e-9 for value in clock.sleeps
    )
    assert sum(clock.sleeps) >= G078_MIN_READ_INTERVAL_SECONDS * 2


def test_process_lock_held_rejected(tmp_path):
    tmp_path.mkdir(parents=True, exist_ok=True)
    lock_path = tmp_path / G078_PROCESS_LOCK_FILE
    holder = lock_path.open("a+", encoding="utf-8")
    fcntl.flock(holder.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    try:
        with pytest.raises(G078Error, match="G078_PROCESS_LOCK_HELD"):
...
def test_non_fake_read_back_client_rejected(tmp_path):
    ...
    with pytest.raises(G078Error, match="G078_FAKE_ONLY_READ_BACK_REQUIRED"):
...
def test_each_source_read_at_most_once(tmp_path):
    ...
    assert g078_resolution_status(state_root=tmp_path)["resolutions_used"] == 1


def test_evidence_artifact_digest_and_binding(tmp_path):
    _run(tmp_path, _executed_entry_client())
    evidence = _evidence(tmp_path)
    assert evidence["schema"] == "H11_V4_G078_RESOLUTION_EVIDENCE_V1"
    assert evidence["generation_label"] == G078_GENERATION_LABEL
...
def test_read_back_contract_violation_is_halt_and_rejected(tmp_path):
    ...
    with pytest.raises(G078Error, match="G078_READ_BACK_CONTRACT_INVALID"):
        _run(tmp_path, G078FakeOnlyCallable(bad_client))
    assert (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_RESOLUTION_INTERNAL_FAILURE"


def test_sanitized_read_is_never_truthy():
    read = _read(source=G078ReadBackSource.LATEST_EXECUTIONS, known=True, count=5)
...
def test_exit_kind_semantics(tmp_path):
    ...
    executed = _client(
        positions=_read(
            source=G078ReadBackSource.OPEN_POSITIONS, known=True, account_flat=True
        ),
        orders=_read(
            source=G078ReadBackSource.ACTIVE_ORDERS,
            known=True,
    ...
    assert _run(
        tmp_path, executed, action_kind=G078WriteActionKind.POSITION_SPECIFIC_EXIT
    ) == G078ResolutionState.CONFIRMED_EXECUTED.value
    assert not (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    ...
    # fourth scope would raise G078_RESOLUTION_BUDGET_EXCEEDED (covered by
...
def test_oco_kind_semantics(tmp_path):
    ...
    assert _run(
        tmp_path, protected, action_kind=G078WriteActionKind.EXACT_SIZE_OCO_PROTECTION
    ) == G078ResolutionState.CONFIRMED_EXECUTED.value
    assert not (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    ...
    assert _run(
        tmp_path,
        flat,
        action_kind=G078WriteActionKind.EXACT_SIZE_OCO_PROTECTION,
        scope=SCOPE_B,
    ) == G078ResolutionState.CONFIRMED_NOT_EXECUTED.value
...
def test_cancel_kind_semantics(tmp_path):
    ...
    assert _run(
        tmp_path, cancelled, action_kind=G078WriteActionKind.CANCEL_UNFILLED_REMAINDER
    ) == G078ResolutionState.CONFIRMED_EXECUTED.value
    assert not (tmp_path / G078_PERSISTENT_HALT_FILE).exists()
    pending = _client(
        orders=_read(
    ...
    assert _run(
        tmp_path,
        pending,
        action_kind=G078WriteActionKind.CANCEL_UNFILLED_REMAINDER,
        scope=SCOPE_B,
    ) == G078ResolutionState.CONFIRMED_NOT_EXECUTED.value
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
    assert halt["reason"] == "G078_RESOLUTION_TERMINAL"
...
def test_initial_activation_kind_is_terminal(tmp_path):
    client = _executed_entry_client()
    assert _run(
        tmp_path, client, action_kind=G078WriteActionKind.INITIAL_ACTIVATION
    ) == G078ResolutionState.UNRESOLVED.value
    halt = json.loads(
        (tmp_path / G078_PERSISTENT_HALT_FILE).read_text(encoding="utf-8")
    )
...
def test_status_projection_without_evidence(tmp_path):
    status = g078_resolution_status(state_root=tmp_path)
    assert status["resolution_state"] == G078ResolutionState.NOT_REQUIRED.value
...
def test_status_projection_with_evidence(tmp_path):
    _run(tmp_path, _executed_entry_client())
    status = g078_resolution_status(state_root=tmp_path)
    assert status["resolution_state"] == G078ResolutionState.CONFIRMED_EXECUTED.value
    assert status["evidence_present"] is True
    assert status["resolutions_used"] == 1
    assert status["resolutions"][0]["scope_token"] == SCOPE[7:]
    assert status["resolutions"][0]["status"] == G078ResolutionState.CONFIRMED_EXECUTED.value


def test_status_projection_with_invalid_evidence(tmp_path):
    tmp_path.mkdir(parents=True, exist_ok=True)
    (tmp_path / f"g078-resolution.{'f' * 64}{G078_RESOLUTION_EVIDENCE_SUFFIX}").write_text(
        '{"schema":"wrong","status":"CONFIRMED_EXECUTED"}', encoding="utf-8"
    )
    status = g078_resolution_status(state_root=tmp_path)
    assert status["resolution_state"] == G078ResolutionState.UNRESOLVED.value
    assert status["resolutions"][0]["evidence_invalid"] is True


def test_module_source_scan_no_external_paths():
    module = (
        REPOSITORY / "backend/app/services/h11_v4_g078_runtime.py"
...
def test_reviewed_digest_self_consistent():
    reviewed = compute_g078_reviewed_files_digest(repository=REPOSITORY)
    assert reviewed.startswith("sha256:")
    artifact_path = REPOSITORY / "docs/templates/h11_v4_g078_frozen_generation.json"
    ...
    try:
        ...
        assert compute_g078_reviewed_files_digest(repository=REPOSITORY) == reviewed
    finally:
...
def test_generation_digest_ignores_binding_fields():
    first = compute_g078_generation_digest(repository=REPOSITORY)
    assert first.startswith("sha256:")
    artifact_path = REPOSITORY / "docs/templates/h11_v4_g078_frozen_generation.json"
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert artifact["generation_label"] == G078_GENERATION_LABEL
    original = artifact_path.read_text(encoding="utf-8")
    ...
    try:
        ...
        assert compute_g078_generation_digest(repository=REPOSITORY) == first
    finally:
        artifact_path.write_text(original, encoding="utf-8")
