"""Fake-only tests for the final G074 runtime connection contract."""
...
class FakeReconciler:
    def __init__(self, snapshot: G074SanitizedSnapshot):
...
class FakePort:
    def __init__(self, outcomes: dict[G074Action, G074ActionOutcome]):
        self.outcomes = outcomes
        self.calls: list[G074Action] = []
...
def snapshot(*, position: int = 0, protected: bool = False) -> G074SanitizedSnapshot:
...
def bind_capability(root: Path) -> None:
    base = {
        ...
        "generation_label": "H11_AUTO_30M_20260802_G074",
        "generation_digest": GEN,
        "reviewed_files_digest": REVIEWED,
    ...
    (root / "g074-switch-control-capability.json").write_text(
        json.dumps({**base, "artifact_digest": digest})
    )
    (root / "g074-release-capability.json").write_text(
        json.dumps({**base, "artifact_digest": digest})
    )
    outcome = {
        "status": "PASSED",
    ...
    (root / "g074-initial-activation.outcome.json").write_text(
        json.dumps({**outcome, "artifact_digest": outcome_digest})
    )
    (root / "g074-operation-60.result.json").write_text(
        json.dumps(
            {
                "status": "PASSED",
                "generation_label": "H11_AUTO_30M_20260802_G074",
...
def test_cycle_is_single_attempt_and_flat(tmp_path: Path):
    ...
    evidence = run_g074_reconciliation_cycle_once(
        state_root=tmp_path,
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
        cycle_id="cycle-1",
    ...
    assert evidence.state is G074ReconciliationState.FRESH_FLAT
    assert runner.calls == 1
    with pytest.raises(G074Error, match="ALREADY_STARTED_NO_RETRY"):
        run_g074_reconciliation_cycle_once(
            state_root=tmp_path,
            generation_digest=GEN,
            reviewed_files_digest=REVIEWED,
...
def test_supervisor_connects_reconciliation_and_strategy_without_transport(tmp_path: Path):
    ...
    runner = FakeReconciler(snapshot())
    evaluator = G074FrozenStrategyEvaluator(
    ...
    supervisor = G074ResidentSupervisor(
        ...
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
        reconciliation_runner=lambda cycle, now: run_g074_reconciliation_cycle_once(
            state_root=tmp_path,
            generation_digest=GEN,
            reviewed_files_digest=REVIEWED,
            cycle_id=cycle,
    ...
    assert status["effective_state"] == G074EffectiveState.ON_WAITING.value
    assert status["entry_gate_open"] is True
    assert status["entry_state"] == G074EntryState.ENTRY_READY.value
...
def test_action_dispatchers_are_one_shot_fake_only(tmp_path: Path):
    port = FakePort(
        {
            G074Action.ENTRY: G074ActionOutcome.ACCEPTED,
            G074Action.PROTECTION: G074ActionOutcome.PROTECTED,
            G074Action.CANCEL_PROTECTION: G074ActionOutcome.ACCEPTED,
            G074Action.CLOSE_POSITION: G074ActionOutcome.FLAT,
        }
    )
    ...
    entry = G074EntryDispatcher(actions=actions)
    decision = G074FrozenStrategyEvaluator(
        source=FakeSource(actionable=True),
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
        strategy_artifact_digest=STRATEGY,
    ...
    assert port.calls == [G074Action.ENTRY, G074Action.PROTECTION]
    exit_dispatcher = G074ExitDispatcher(
        actions=G074OneShotActionDispatcher.bound(
    ...
    protected = run_g074_reconciliation_cycle_once(
    ...
    exit_dispatcher.exit_once(
        ...
        cycle_id="action-2",
        side="BUY",
        reason=G074Action.TIME_EXIT,
        now_utc=NOW,
    )
    assert port.calls[-2:] == [G074Action.CANCEL_PROTECTION, G074Action.CLOSE_POSITION]


def test_restart_health_and_arm_off_are_fail_closed(tmp_path: Path):
    bind_capability(tmp_path)
    status = G074ResidentSupervisor(
        state_root=tmp_path,
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
...
def test_initial_activation_is_one_shot_and_enables_switch_only_after_pass(tmp_path: Path):
    ...
    (tmp_path / "g074-operation-60.result.json").write_text(
        json.dumps(
            {
                "status": "PASSED",
                "generation_label": "H11_AUTO_30M_20260802_G074",
    ...
    with pytest.raises(G074Error, match="ONE_USE_MARKER"):
        run_g074_initial_atomic_activation(
            state_root=tmp_path,
            generation_digest=GEN,
            reviewed_files_digest=REVIEWED,
...
def test_recovery_scope_requires_explicit_owned_quantity_and_protection(tmp_path: Path):
    ...
    scope = build_g074_recovery_scope(
        state_root=tmp_path,
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
        evidence=protected,
    ...
    unconfirmed = replace(
        protected,
        state=G074ReconciliationState.UNKNOWN,
        ownership_exact=False,
        quantity_matches=False,
        protection_confirmed=False,
    )
    with pytest.raises(G074Error, match="RECOVERY_EVIDENCE_NOT_CLEAR"):
        build_g074_recovery_scope(
            state_root=tmp_path,
            generation_digest=GEN,
            reviewed_files_digest=REVIEWED,
...
def test_resident_cycle_ids_do_not_collide_across_restarts(tmp_path: Path):
    first = G074ResidentSupervisor(
        state_root=tmp_path,
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
    )
    second = G074ResidentSupervisor(
        state_root=tmp_path,
        generation_digest=GEN,
        reviewed_files_digest=REVIEWED,
    )
...
def test_capability_rejects_mismatched_operation_binding(tmp_path: Path):
    ...
    bind_capability(tmp_path)
    operation = json.loads((tmp_path / "g074-operation-60.result.json").read_text())
    operation["generation_digest"] = "sha256:" + "9" * 64
    (tmp_path / "g074-operation-60.result.json").write_text(json.dumps(operation))
    status = safe_g074_api_status(
        state_root=tmp_path,
        arm_on=False,
        generation_digest=GEN,
...
def test_recovery_scope_is_action_bound_and_one_use(tmp_path: Path):
    ...
    build_g074_recovery_scope(**kwargs)
    with pytest.raises(G074Error, match="ONE_USE_MARKER"):
        build_g074_recovery_scope(**kwargs)
...
def test_scheduler_binding_requires_exact_arguments_live_pid_and_chain(tmp_path: Path):
    ...
    arguments = [
        ...
        str(repository / "backend/scripts/h11_auto_v4_g074_runtime_bootstrap.py"),
        "--repository",
        str(repository),
    ...
    with pytest.raises(G074Error, match="SCHEDULER_BINDING_INVALID"):
        verify_g074_scheduler_binding(
            generation=generation,
            repository=repository,
            plist_path=plist,
    ...
    with pytest.raises(G074Error, match="RUNTIME_READINESS_NOT_CLEAR"):
        verify_g074_scheduler_binding(
            generation=generation,
            repository=repository,
            plist_path=plist,
...
def test_operation_60_renders_the_exact_workspace_interpreter():
    repository = Path(__file__).resolve().parents[4]
    source = (
        repository / "backend/scripts/h11_auto_v4_g074_operation_60_no_post.py"
    ).read_text()
