"""Step 6G controlled adapter skeleton with fake transport only.

This module models the adapter boundary that may precede a future real Step 6G
execution adapter. In this step it accepts only fake transport evidence. It does
...
LIVE_ORDER_REAL_STEP6G_CONTROLLED_ADAPTER_ID_PREFIX = "LOR6GAD-"
STEP6G_CONTROLLED_ADAPTER_RECOMMENDED_NEXT_STEP = (
    "implement_separate_real_adapter_step_with_new_final_confirmation_fresh_preflight_and_no_retry"
)


class LiveOrderRealStep6GControlledAdapterStatus(str, Enum):
    ...
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_INPUT_NOT_READY = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_INPUT_NOT_READY"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_TRANSPORT_UNSAFE = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_TRANSPORT_UNSAFE"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_REAL_TRANSPORT_NOT_ALLOWED = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_REAL_TRANSPORT_NOT_ALLOWED"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_ATTEMPT_STATE = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_ATTEMPT_STATE"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_RAW_OR_SECRET_EXPOSURE = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_RAW_OR_SECRET_EXPOSURE"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_RETRY_OR_LOOP = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_RETRY_OR_LOOP"
    )
    BLOCKED_STEP6G_CONTROLLED_ADAPTER_UNSUPPORTED = (
        "BLOCKED_STEP6G_CONTROLLED_ADAPTER_UNSUPPORTED"
    )


class LiveOrderRealStep6GControlledAdapterMode(str, Enum):
    FAKE_TRANSPORT_ONLY_NO_API_NO_POST = "FAKE_TRANSPORT_ONLY_NO_API_NO_POST"


class LiveOrderRealStep6GControlledTransportMode(str, Enum):
    FAKE_ONLY = "FAKE_ONLY"
    REAL_TRANSPORT = "REAL_TRANSPORT"


class LiveOrderRealStep6GControlledTransportResultCategory(str, Enum):
...
AdapterStatus = LiveOrderRealStep6GControlledAdapterStatus
AdapterMode = LiveOrderRealStep6GControlledAdapterMode
TransportMode = LiveOrderRealStep6GControlledTransportMode
TransportResultCategory = LiveOrderRealStep6GControlledTransportResultCategory


@dataclass(frozen=True)
class LiveOrderRealStep6GControlledAdapterRequest:
    source_bridge_id: str
    source_bridge_status: str
    source_runtime_bridge_id: str
    source_runtime_bridge_status: str
    adapter_mode: LiveOrderRealStep6GControlledAdapterMode
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.adapter_mode, LiveOrderRealStep6GControlledAdapterMode):
...
@dataclass(frozen=True)
class LiveOrderRealStep6GControlledTransportContract:
    transport_mode: LiveOrderRealStep6GControlledTransportMode
    transport_name: str
    is_fake_transport: bool
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.transport_mode, LiveOrderRealStep6GControlledTransportMode):
...
@dataclass(frozen=True)
class LiveOrderRealStep6GControlledTransportResult:
    fake_transport_attempted: bool
    fake_transport_result_category: LiveOrderRealStep6GControlledTransportResultCategory
    fake_attempt_count: int
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.fake_transport_result_category,
            LiveOrderRealStep6GControlledTransportResultCategory,
        ):
            raise LiveVerificationValidationError(
                "fake_transport_result_category must be Step 6G fake adapter category",
            )
        _validate_non_negative_int("fake_attempt_count", self.fake_attempt_count)
...
@dataclass(frozen=True)
class LiveOrderRealStep6GControlledAdapterCheckResult:
    name: str
    passed: bool
    reason: str
    sanitized_value: str
...
@dataclass(frozen=True)
class LiveOrderRealStep6GControlledAdapterResult:
    adapter_id: str
    created_at: datetime
    status: LiveOrderRealStep6GControlledAdapterStatus
    fake_adapter_ready: bool
    fake_transport_attempted: bool
    fake_transport_result_category: str
    real_http_post_executed: bool
    order_endpoint_called: bool
    live_order_once_called: bool
    broker_order_path_called: bool
    ...
    request: LiveOrderRealStep6GControlledAdapterRequest
    transport_contract: LiveOrderRealStep6GControlledTransportContract
    transport_result: LiveOrderRealStep6GControlledTransportResult | None
    check_results: tuple[LiveOrderRealStep6GControlledAdapterCheckResult, ...]
    blocked_reasons: tuple[str, ...]

    @property
    def adapter_status(self) -> LiveOrderRealStep6GControlledAdapterStatus:
        return self.status

    def __post_init__(self) -> None:
        _require_non_empty("adapter_id", self.adapter_id)
        _ensure_aware(self.created_at)
        if not isinstance(self.status, LiveOrderRealStep6GControlledAdapterStatus):
        ...
        _require_non_empty("fake_transport_result_category", self.fake_transport_result_category)
        ...
        if self.fake_adapter_ready and self.blocked_reasons:
            raise LiveVerificationValidationError("ready fake adapter cannot have blockers")
        if self.real_http_post_executed:
            raise LiveVerificationValidationError(
                "fake adapter must keep real_http_post_executed=false",
            )
        if self.order_endpoint_called:
            raise LiveVerificationValidationError(
                "fake adapter must keep order_endpoint_called=false",
            )
        if self.live_order_once_called:
            raise LiveVerificationValidationError(
                "fake adapter must keep live_order_once_called=false",
            )
        if self.broker_order_path_called:
            raise LiveVerificationValidationError(
                "fake adapter must keep broker_order_path_called=false",
            )
        if self.allowed_for_live:
            raise LiveVerificationValidationError("fake adapter must keep allowed_for_live=false")
        if self.post_allowed_this_step:
            raise LiveVerificationValidationError(
                "fake adapter must keep post_allowed_this_step=false",
            )
        if self.post_executed:
            raise LiveVerificationValidationError("fake adapter must keep post_executed=false")


def build_live_order_real_step6g_controlled_adapter(
    *,
    ...
    transport_contract: LiveOrderRealStep6GControlledTransportContract,
    adapter_request: LiveOrderRealStep6GControlledAdapterRequest | None = None,
    transport_result: LiveOrderRealStep6GControlledTransportResult | None = None,
    created_at: datetime | None = None,
) -> LiveOrderRealStep6GControlledAdapterResult:
    """Build a fake-transport-only controlled adapter decision."""
    created = _ensure_aware(created_at or datetime.now(UTC))
    request = adapter_request or make_live_order_real_step6g_controlled_adapter_request(
        step6g_post_route_bridge_result=step6g_post_route_bridge_result,
        step6g_runtime_bridge_result=step6g_runtime_bridge_result,
        fake_attempt_count=transport_result.fake_attempt_count
        if transport_result
        else step6g_runtime_bridge_result.fake_attempt_count,
    )
    ...
    if input_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_INPUT_NOT_READY
        primary_reasons = input_reasons
    elif real_transport_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_REAL_TRANSPORT_NOT_ALLOWED
        primary_reasons = real_transport_reasons
    elif attempt_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_ATTEMPT_STATE
        primary_reasons = attempt_reasons
    elif retry_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_RETRY_OR_LOOP
        primary_reasons = retry_reasons
    elif raw_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_RAW_OR_SECRET_EXPOSURE
        primary_reasons = raw_reasons
    elif transport_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_TRANSPORT_UNSAFE
        primary_reasons = transport_reasons
    elif unsupported_reasons:
        status = AdapterStatus.BLOCKED_STEP6G_CONTROLLED_ADAPTER_UNSUPPORTED
        primary_reasons = unsupported_reasons
    elif transport_result is None:
        status = AdapterStatus.STEP6G_CONTROLLED_ADAPTER_FAKE_READY_NO_API_NO_POST
        primary_reasons = ()
    else:
        status = AdapterStatus.STEP6G_CONTROLLED_ADAPTER_FAKE_COMPLETED_NO_API_NO_POST
        primary_reasons = ()
    ...
    ready = status in {
        AdapterStatus.STEP6G_CONTROLLED_ADAPTER_FAKE_READY_NO_API_NO_POST,
        AdapterStatus.STEP6G_CONTROLLED_ADAPTER_FAKE_COMPLETED_NO_API_NO_POST,
    }
    ...
    return LiveOrderRealStep6GControlledAdapterResult(
        adapter_id=make_live_order_real_step6g_controlled_adapter_id(created),
        created_at=created,
        status=status,
        fake_adapter_ready=ready,
        fake_transport_attempted=bool(
            transport_result and transport_result.fake_transport_attempted
        ),
        fake_transport_result_category=transport_result.fake_transport_result_category.value
        if transport_result
        else "NOT_RUN_FAKE_TRANSPORT_ONLY_NO_API_NO_POST",
        ...
        recommended_next_step=STEP6G_CONTROLLED_ADAPTER_RECOMMENDED_NEXT_STEP
        if ready
        else "fix_step6g_controlled_adapter_blockers_no_api_no_post",
        request=request,
        transport_contract=transport_contract,
        transport_result=transport_result,
...
def run_live_order_real_step6g_controlled_adapter_with_fake_transport(
    *,
    step6g_post_route_bridge_result: LiveOrderRealStep6GPostRouteBridgeResult,
    step6g_runtime_bridge_result: LiveOrderRealStep6GRuntimeBridgeResult,
    transport_contract: LiveOrderRealStep6GControlledTransportContract | None = None,
    fake_transport_result_category: LiveOrderRealStep6GControlledTransportResultCategory = (
    ...
    """Run the fake-transport-only adapter skeleton without API or POST execution."""
    contract = transport_contract or make_live_order_real_step6g_fake_transport_contract()
    request = adapter_request or make_live_order_real_step6g_controlled_adapter_request(
        step6g_post_route_bridge_result=step6g_post_route_bridge_result,
        step6g_runtime_bridge_result=step6g_runtime_bridge_result,
        fake_attempt_count=1,
    )
    transport_result = LiveOrderRealStep6GControlledTransportResult(
        fake_transport_attempted=True,
        fake_transport_result_category=fake_transport_result_category,
...
def make_live_order_real_step6g_fake_transport_contract(
    *,
    transport_name: str = "step6g_controlled_fake_transport_no_api_no_post",
) -> LiveOrderRealStep6GControlledTransportContract:
    return LiveOrderRealStep6GControlledTransportContract(
        transport_mode=TransportMode.FAKE_ONLY,
...
def make_live_order_real_step6g_controlled_adapter_request(
    *,
    step6g_post_route_bridge_result: LiveOrderRealStep6GPostRouteBridgeResult,
    step6g_runtime_bridge_result: LiveOrderRealStep6GRuntimeBridgeResult,
    fake_attempt_count: int | None = None,
    ...
    return LiveOrderRealStep6GControlledAdapterRequest(
        ...
        adapter_mode=AdapterMode.FAKE_TRANSPORT_ONLY_NO_API_NO_POST,
        ...
        fake_attempt_count=runtime.fake_attempt_count
        if fake_attempt_count is None
        else fake_attempt_count,
        post_allowed_this_step=bridge.post_allowed_this_step or runtime.post_allowed_this_step,
        post_executed=bridge.post_executed or runtime.post_executed,
        allowed_for_live=bridge.allowed_for_live or runtime.allowed_for_live,
        allowed_for_live_persisted=attempt.allowed_for_live_persisted,
        retry_allowed=attempt.retry_allowed or runtime.request.retry_allowed,
        loop_allowed=attempt.loop_allowed or runtime.request.loop_allowed,
        add_order_allowed=attempt.add_order_allowed or runtime.request.add_order_allowed,
        change_order_allowed=attempt.change_order_allowed or runtime.request.change_order_allowed,
        cancel_order_allowed=attempt.cancel_order_allowed or runtime.request.cancel_order_allowed,
        close_order_allowed=attempt.close_order_allowed or runtime.request.close_order_allowed,
        raw_secret_id_exposure=runtime.request.raw_secret_id_exposure
        or _source_has_raw_or_secret_exposure(bridge),
        route_unsafe=runtime.request.route_unsafe or _source_route_is_unsafe(bridge),
        step4_spoofing=runtime.request.step4_spoofing or _source_has_step4_spoofing(bridge),
        real_http_post_executed=(
...
def render_live_order_real_step6g_controlled_adapter_markdown(
    result: LiveOrderRealStep6GControlledAdapterResult,
) -> str:
    """Render a sanitized fake controlled adapter report."""
    lines = [
        "# Step 6G Controlled Adapter Fake Transport",
        "",
        "This controlled adapter is fake transport only.",
        "This controlled adapter does not execute API calls.",
        "This controlled adapter does not execute HTTP POST.",
        "This controlled adapter does not call order endpoint.",
        "This controlled adapter does not call live_order_once.",
        "This controlled adapter does not reuse old final confirmation.",
        "Future real Step 6G execution requires a new final confirmation and fresh preflight.",
        "",
        "## Summary",
        f"- status: {result.status.value}",
        f"- fake_adapter_ready: {_bool_text(result.fake_adapter_ready)}",
        f"- fake_transport_attempted: {_bool_text(result.fake_transport_attempted)}",
        f"- fake_transport_result_category: {result.fake_transport_result_category}",
        ...
        f"- is_fake_transport: {_bool_text(result.transport_contract.is_fake_transport)}",
...
def make_live_order_real_step6g_controlled_adapter_id(created_at: datetime) -> str:
    created = _ensure_aware(created_at)
    return (
        f"{LIVE_ORDER_REAL_STEP6G_CONTROLLED_ADAPTER_ID_PREFIX}"
...
def _real_transport_reasons(
    contract: LiveOrderRealStep6GControlledTransportContract,
) -> tuple[str, ...]:
    reasons: list[str] = []
    if contract.transport_mode is not TransportMode.FAKE_ONLY:
        reasons.append("transport_mode_not_fake_only")
    if not contract.is_fake_transport:
        reasons.append("transport_not_fake")
...
def _attempt_state_reasons(
    ...
    if request.post_allowed_this_step:
        reasons.append("post_allowed_this_step_unsafe_for_fake_adapter")
    ...
    if transport_result and transport_result.fake_attempt_count > 1:
        reasons.append("fake_transport_attempt_count_exceeds_one")
    return tuple(reasons)
...
def _retry_or_loop_reasons(
    ...
    if transport_result:
        if transport_result.fake_retry_count:
            reasons.append("fake_transport_retry_count_non_zero")
        if transport_result.fake_loop_count:
            reasons.append("fake_transport_loop_count_non_zero")
    return tuple(reasons)
    ...
    if transport_result:
        for field_name in (
            ...
            if getattr(transport_result, field_name):
                reasons.append(f"fake_transport_{field_name}_unsafe")
    return tuple(reasons)
...
def _raw_or_secret_exposure_reasons(
    ...
    if transport_result:
        for field_name in (
            ...
            if getattr(transport_result, field_name):
                reasons.append(f"fake_transport_{field_name}_unsafe")
        if not transport_result.result_is_fake:
            reasons.append("fake_transport_result_not_fake")
    return tuple(reasons)


def _unsupported_reasons(request: LiveOrderRealStep6GControlledAdapterRequest) -> tuple[str, ...]:
    if request.adapter_mode is AdapterMode.FAKE_TRANSPORT_ONLY_NO_API_NO_POST:
        return ()
...
def _build_check_results(
    *,
    ...
    transport_contract: LiveOrderRealStep6GControlledTransportContract,
    transport_result: LiveOrderRealStep6GControlledTransportResult | None,
) -> tuple[LiveOrderRealStep6GControlledAdapterCheckResult, ...]:
    checks: list[LiveOrderRealStep6GControlledAdapterCheckResult] = []
    ...
    def add(name: str, passed: bool, value: object, expected: str) -> None:
        checks.append(
            LiveOrderRealStep6GControlledAdapterCheckResult(
                name=name,
                passed=passed,
    ...
    add(
        "transport fake only",
        transport_contract.transport_mode is TransportMode.FAKE_ONLY
        and transport_contract.is_fake_transport
        and not transport_contract.is_real_transport,
        transport_contract.transport_mode.value,
        TransportMode.FAKE_ONLY.value,
    )
    add(
    ...
    add(
        "fake result stays fake",
        transport_result is None
        or (transport_result.fake_transport_attempted and transport_result.result_is_fake),
        "fake" if transport_result else "not_run",
        "fake or not_run",
    )
    return tuple(checks)
