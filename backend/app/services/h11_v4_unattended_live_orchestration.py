...
decision logic and no state of its own — beyond its own fail-closed
credential/client guard, every gate it relies on lives, unchanged and
already reviewed, inside the two functions it calls.

``notification_ready``/``entry_gate_blocked_reasons`` are caller-supplied,
unverifiable claims at this layer (design doc §9.2 item 4, §10.2): the
future CLI/wiring slice must derive both from real same-cycle evaluations
— never hardcode ``True``/``()`` the way this module's own tests do for
fake-only isolation.
...
bypass-prevention obligation.  ``credential_pair``/``client`` are required
with no default of any kind; no real Keychain access exists here, and
nothing is wired into any runtime, CLI, or scheduler.
"""
...
def run_unattended_live_entry_cycle_once(
    *,
    ...
    """Run at most one unattended entry cycle: verify, mint, then drive.

    Sequence (§11.6, nothing added or reordered):
    ...
    2. ``confirm_v4_unattended_authorization_once`` with ``session.intent``
       and the caller-supplied stores.  This re-verifies all six conditions
       fresh, consumes the operator's daily authorization as its first
       write, and mints the resume/current-turn proof pair — or raises with
       nothing minted.
    3. ``run_g013_actual_canary_after_unattended_authorization`` with those
    ...
    Known, accepted cost (§10.3/§11.6): the daily authorization is consumed
    at step 2, so a driver failure at step 3 burns the day with no entry —
    the track-wide fail-closed preference over any double-issuance risk.
    """
