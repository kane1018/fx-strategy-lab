"""One-use G074 release activation: three read-only GETs then local ARM ON."""
...
from app.services.h11_v4_g074_private_get_keychain import (  # noqa: E402
    G074PrivateGetKeychainCredentialPair,
)
...
def main() -> int:
    ...
    if not args.execute_final_transaction:
        raise G074Error("G074_INITIAL_TRANSACTION_EXPLICIT_FLAG_REQUIRED")
    ...
    if generation.generation_label != G074_GENERATION_LABEL:
        raise G074Error("G074_CANONICAL_GENERATION_REQUIRED")
    verify_g074_review_artifacts(
    ...
    def reconcile_once():
        with httpx.Client(timeout=5.0) as client:
            snapshot = read_v4_unattended_shadow_private_snapshot(
                credential_pair=G074PrivateGetKeychainCredentialPair(), client=client
            )
        if not snapshot.account_flat or not snapshot.active_orders_zero:
            raise G074Error("G074_INITIAL_ACCOUNT_NOT_FLAT")
        return run_g074_reconciliation_cycle_once(
            state_root=state_root,
            generation_digest=generation.digest,
            reviewed_files_digest=reviewed,
            cycle_id="initial-activation",
            reconciler=lambda **_kwargs: G074SanitizedSnapshot(
    ...
    outcome = run_g074_initial_atomic_activation(
        state_root=state_root,
        generation_digest=generation.digest,
        reviewed_files_digest=reviewed,
    ...
    print(f"G074_INITIAL_ATOMIC_ACTIVATION_{outcome}")
    return 0
