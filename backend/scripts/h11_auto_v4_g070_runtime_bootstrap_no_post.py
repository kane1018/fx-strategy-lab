"""G070 resident candidate; no external adapter is constructed by this entrypoint."""
...
from app.services.h11_v4_g070_candidate import (  # noqa: E402
    ArmState,
    G070OwnerLock,
...
def run_resident_candidate(
    *,
    ...
    lock = G070OwnerLock(state_root / "process.lock")
    lock.acquire()
    supervisor = G070ResidentSupervisor(state_root, generation_digest, reviewed_files_digest)
    completed = 0
    try:
        ...
        engage_g070_halt(state_root=state_root, reason="RESIDENT_TERMINATED_UNKNOWN")
        raise
    finally:
        lock.release()
...
def main() -> int:
    ...
    run_resident_candidate(
        ...
        reviewed_files_digest=args.expected_reviewed_files_digest,
