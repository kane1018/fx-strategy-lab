#!/usr/bin/env python3
"""G068 resident local-health bootstrap; no broker or credential access."""
...
from app.services.h11_v4_g068_unattended_activation_no_post import (  # noqa: E402
    ...
    write_g068_runtime_projection_no_post,
)
...
def _load_verified_generation(*, repository: Path, generation_digest: str, reviewed_digest: str):
    ...
    if computed != reviewed_digest:
        raise V4G068ActivationError("G068_REVIEWED_FILES_DIGEST_MISMATCH")
    generation = load_v4_gmo_frozen_generation(
        repository=repository, implementation_digest=computed
    )
    if generation.digest != generation_digest:
        raise V4G068ActivationError("G068_GENERATION_DIGEST_MISMATCH")
...
def run_once(*, repository: Path, generation_digest: str, reviewed_digest: str) -> int:
    ...
    lock = G068OwnerLock(root / "process.lock", generation_digest=generation_digest)
    try:
        lock.acquire()
        write_g068_health_no_post(
            state_root=root,
        ...
        return 0
    except V4G068ActivationError as error:
        try:
            write_g068_persistent_halt_no_post(
                state_root=root,
                generation_digest=generation_digest,
...
def run_resident(*, repository: Path, generation_digest: str, reviewed_digest: str) -> int:
    ...
    try:
        ...
        while True:
            ...
            time.sleep(15)
    except (KeyboardInterrupt, V4G068ActivationError) as error:
        try:
            write_g068_persistent_halt_no_post(
                state_root=root,
                generation_digest=generation_digest,
