#!/usr/bin/env python3
"""Run the G056 generation-bound one-use manual-flat Private GET snapshot."""
...
from app.h11_auto.v4_gmo_launchd import require_stable_v4_gmo_aqua_domain
...
_EXPECTED_GENERATION_LABEL = "H11_AUTO_30M_20260730_G056"


def main() -> int:
    repository = Path(__file__).resolve().parents[2]
    try:
        _require_gui_capable_context()
        ...
        if generation.generation_label != _EXPECTED_GENERATION_LABEL:
            raise RuntimeError("G056_SNAPSHOT_GENERATION_INVALID")
...
def _require_gui_capable_context() -> None:
...
def _safe_failure() -> dict[str, object]:
    return {
        "status": "G056_MANUAL_FLAT_SNAPSHOT_REFUSED_OR_FAILED_NO_RETRY",
        "account_flat": False,
...
def _require_clean_main(repository: Path) -> None:
    def git(*arguments: str) -> str:
        ...
        if result.returncode != 0:
            raise RuntimeError("G056_REPOSITORY_GATE_FAILED")
