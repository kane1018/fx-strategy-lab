#!/usr/bin/env python3
"""Run the G054 generation-bound one-use manual-flat Private GET snapshot."""
...
_EXPECTED_GENERATION_LABEL = "H11_AUTO_30M_20260730_G054"
...
def main() -> int:
    ...
    try:
        ...
        if generation.generation_label != _EXPECTED_GENERATION_LABEL:
            raise RuntimeError("G054_SNAPSHOT_GENERATION_INVALID")
...
def _safe_failure() -> dict[str, object]:
    return {
        "status": "G054_MANUAL_FLAT_SNAPSHOT_REFUSED_OR_FAILED_NO_RETRY",
        "account_flat": False,
...
def _require_clean_main(repository: Path) -> None:
    def git(*arguments: str) -> str:
        ...
        if result.returncode != 0:
            raise RuntimeError("G054_REPOSITORY_GATE_FAILED")
