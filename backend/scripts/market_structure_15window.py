"""15-window evaluation of market-structure mean-reversion (read-only, paper).

Final simple-structure test. rsi/breakout/bollinger all failed at M5/current_cost;
the mean-reversion edge is real only in low-DE chop but is eaten by the trend-day
SL tail. Hypothesis: entering reversion ONLY at recent swing extremes (lookback 12)
with a rejection wick (>=40% of range) cuts trade count and avoids trend-initiation
entries, lightening the SL tail vs rsi/bollinger.

FIXED (no tuning): swing lookback 12, wick ratio 0.40, M5, current_cost (spread 1.2
/ slippage 0.2), baseline exit, SL 30 / TP 60, continuous replay, no ADX/DE filter.

Reuses the bollinger/rsi/breakout/market-state pipelines. Also replays rsi + breakout
per day to attribute results to market-state buckets (DE tertiles, day class).

No real orders, no Private API, no API key/secret. In-memory DBs only.

  .venv/bin/python -m scripts.market_structure_15window
"""
...
from scripts.bollinger_15window import (  # noqa: E402
    DAY_CLASSES,
    DE_BUCKETS,
    _replay_full_trades,
    _tag_market_state,
...
from scripts.robustness_windows import _OPP, _SL, _weekdays, robustness_summary  # noqa: E402
...
SWING_LOOKBACK, WICK_RATIO = 12, 0.4

# Reference results from committed 15-window runs for side-by-side comparison.
RSI_REF = {"median_expectancy": 0.0164, "median_pf": 1.016, "positive_windows": 8,
           "total_pnl": 56.95, "max_drawdown_max": 65.46}
BOLL_REF = {"median_expectancy": -0.0667, "median_pf": 0.883, "positive_windows": 3,
            "total_pnl": -273.8, "max_drawdown_max": 75.48}
...
def _collect(broker: GmoFxBroker, warnings: list[str]) -> tuple[list[dict], dict]:
    ms = StrategyConfig(strategy_type=StrategyType.MARKET_STRUCTURE_REVERSION)  # lookback12/wick40
    ...
    for label, start, end, group in WINDOWS:
        ...
        for symbol in SYMBOLS:
            ...
            if not candles:
                continue
            pip = pip_size(symbol)
            ms_trades = _replay_full_trades(candles, symbol, ms, execution, fast=False)
        ...
        print(f"{label:>13} {start}-{end} [{group}] market-structure trades={done}")
...
def _export(all_trades: list[dict], day_meta: dict, warnings: list[str]) -> None:
    ...
    (out / "warnings.json").write_text(json.dumps({
        ...
        "fixed_config": f"market_structure(lookback {SWING_LOOKBACK}, wick {WICK_RATIO}) / "
                        ...
                        "baseline exit / SL30 / TP60 / no ADX/DE filter",
        "entry_rule": "swing low/high over last 12 bars + rejection wick>=40% of range; "
                      "confirmed bars only (frame[:index]); no look-ahead",
...
def _build_summary(all_trades, window_stats, win_group, symbol_window_wl, lo, hi) -> dict:
    ...
    summary.update({
        ...
        "de_tertiles": {"low<": lo, "high>=": hi},
        "rsi_ref": RSI_REF, "bollinger_ref": BOLL_REF, "breakout_ref": BK_REF,
    })
    return summary
...
def _write_summary(out, by_window, by_symbol, by_reason, by_state, summary) -> None:
    ...
    rsi, boll, bk = summary["rsi_ref"], summary["bollinger_ref"], summary["breakout_ref"]
    text = (
        "# market-structure mean-reversion M5 15窓評価 (lookback12/wick40%・baseline exit)\n\n"
        "GMO Public klines。実注文なし・Private接続なし・APIキー未使用。"
        "rsi/bollinger/breakoutと同一15窓・同一固定条件。\n\n"
        "## window別\n" + win_h + win_rows + "\n\n"
        "## 15窓全体集計\n"
        ...
        f"総取引: {summary['total_trades']} / 合計損益: {summary['total_pnl']} / "
        f"最大DD最大: {summary['max_drawdown_max']}\n"
        ...
        "|--|--:|--:|--:|--:|--:|\n"
        f"| market_structure | {summary['median_expectancy']} | {summary['median_pf']} | "
        ...
        f"| bollinger | {boll['median_expectancy']} | {boll['median_pf']} | "
        f"{boll['positive_windows']} | {boll['total_pnl']} | {boll['max_drawdown_max']} |\n"
...
def _write_decision(out, summary, verdict, reasons) -> None:
    rsi = summary["rsi_ref"]
    text = (
        "# market-structure mean-reversion M5 最終判断\n\n"
        f"## 判定: {verdict}\n\n"
        ...
        f"- rsi比: 中央値 {summary['median_expectancy']} vs {rsi['median_expectancy']}、"
        f"合計 {summary['total_pnl']} vs {rsi['total_pnl']}\n\n"
        "### 今後の扱い\n"
        "- 継続検証候補: 主検証として残す。\n"
