"""One-use G073 operation 60 with separate installer/readiness windows."""
...
from app.services.h11_v4_g073_runtime import (
    G073_OPERATION_60_RESULT_FILE,
    G073_OPERATION_60_STARTED_FILE,
    G073Error,
    engage_g073_halt,
...
def run_g073_operation_60_candidate(
    *,
    state_root: Path,
    ...
    started = state_root / G073_OPERATION_60_STARTED_FILE
    ...
    result_path = state_root / G073_OPERATION_60_RESULT_FILE
    if result_path.exists() or result_path.is_symlink():
        raise G073Error("G073_OPERATION_60_RESULT_EXISTS_NO_RETRY")
    result_path.write_text(json.dumps(result, sort_keys=True), encoding="utf-8")
    if outcome == "UNKNOWN":
        engage_g073_halt(state_root=state_root, reason="G073_OPERATION_60_UNKNOWN")
    return outcome
...
def _exclusive_start(path: Path, generation_digest: str, reviewed_files_digest: str) -> None:
    ...
    try:
        ...
        raise G073Error("G073_OPERATION_60_ALREADY_STARTED_NO_RETRY") from error
...
def main() -> int:
    ...
    plist = render_v4_gmo_unattended_scheduler_launchagent(
        repository=repository, generation=generation, python_executable=Path(sys.executable)
    )
    ...
    outcome = run_g073_operation_60_candidate(
    ...
    print(f"G073_OPERATION_60_{outcome}_NO_POST")
