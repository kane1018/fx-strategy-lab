"""G072 one-use resident commissioning with separate timeout windows."""
...
from app.services.h11_v4_g072_switch_control import (
    G072_OPERATION_60_RESULT_FILE,
    G072_OPERATION_60_STARTED_FILE,
    G072Error,
    engage_g072_halt,
...
_LAUNCHCTL_TIMEOUT_SECONDS = {"print": 15.0, "bootout": 30.0, "bootstrap": 30.0}
...
def run_g072_operation_60_candidate(
    *,
    state_root: Path,
    ...
    started = state_root / G072_OPERATION_60_STARTED_FILE
    try:
        descriptor = os.open(started, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
    except FileExistsError as error:
        raise G072Error("G072_OPERATION_60_ALREADY_STARTED_NO_RETRY") from error
    with os.fdopen(descriptor, "w", encoding="utf-8") as stream:
        json.dump(
            {
                "generation_label": "H11_AUTO_30M_20260802_G072",
    ...
    try:
        ...
        installer()
        installer_elapsed = monotonic() - installer_started
        if installer_elapsed > installer_timeout_seconds:
            outcome = "UNKNOWN"
        else:
    ...
    result_path = state_root / G072_OPERATION_60_RESULT_FILE
    if result_path.exists() or result_path.is_symlink():
        raise G072Error("G072_OPERATION_60_RESULT_EXISTS_NO_RETRY")
    result_path.write_text(json.dumps(result, sort_keys=True), encoding="utf-8")
    if outcome == "UNKNOWN":
        engage_g072_halt(state_root=state_root, reason="G072_OPERATION_60_UNKNOWN")
    return outcome
...
def main() -> int:
    ...
    outcome = run_g072_operation_60_candidate(
    ...
    print(f"G072_OPERATION_60_{outcome}_NO_POST")
