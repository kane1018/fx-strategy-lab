"""G073 resident runtime bootstrap with no external transport."""
...
from app.services.h11_v4_g073_runtime import (  # noqa: E402
...
def main() -> int:
    ...
    if (
        generation.generation_label != G073_GENERATION_LABEL
    ...
    lock = G073ProcessLock(state_root)
    try:
        lock.acquire()
        ...
        while True:
            ...
            time.sleep(15)
    except (KeyboardInterrupt, G073Error, OSError, ValueError):
        engage_g073_halt(state_root=state_root, reason="G073_RUNTIME_TERMINATED")
        return 4
    finally:
        lock.release()
