"""G071 one-use resident commissioning with no broker or credential access."""
...
from app.services.h11_v4_g071_atomic_activation import (
    ...
    G071_OPERATION_60_STARTED_FILE,
    G071Error,
    engage_g071_halt,
...
def run_g071_operation_60_candidate(
    *,
    state_root: Path,
    ...
    started = state_root / G071_OPERATION_60_STARTED_FILE
    try:
        descriptor = os.open(started, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
    except FileExistsError as error:
        raise G071Error("G071_OPERATION_60_ALREADY_STARTED_NO_RETRY") from error
    ...
    try:
        ...
        outcome = "UNKNOWN"
    (state_root / G071_OPERATION_60_RESULT_FILE).write_text(
        json.dumps(
            {
                "status": outcome,
                "generation_digest": generation_digest,
    ...
    if outcome == "UNKNOWN":
        engage_g071_halt(state_root=state_root, reason="G071_OPERATION_60_UNKNOWN")
    return outcome
...
def main() -> int:
    ...
    print(f"G071_OPERATION_60_{outcome}_NO_POST")
